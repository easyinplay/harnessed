...
**ทำงานอย่างไร**: harnessed **ประกอบ (assembles)** agent โอเพ่นซอร์สที่ดีที่สุดของ Claude Code (gstack, GSD, superpowers, planning-with-files) แล้ว **orchestrate** ให้กลายเป็น workflow เดียวผ่าน composition skills ที่มีจุดยืนชัดเจน มัน **ไม่** vendor โค้ดต้นน้ำ — manifests อธิบาย install/check ส่วน composition skills เป็นผู้ควบคุมความร่วมมือข้าม upstream หลายตัว (ดังนั้นการอัปเกรด upstream จึงเป็นแค่การ re-install ไม่ต้อง sync โค้ดด้วยมือ)

### 🔁 ลูปการทำงาน (operating loop)

> **Discuss → Plan → Build → Verify → Ship** ปิดท้ายด้วยลูป **Learn** — machine-execute ข้าม three-layer stack (gstack governance · GSD orchestration · superpowers TDD · checkpoint evidence) งานของ agent ดิบ ๆ มักไหลเลื่อน (drift); harnessed เปลี่ยนมันเป็นเส้นทางที่เป็น source-of-truth ซึ่งความคืบหน้าและหลักฐานคงอยู่แทนที่จะลอยอยู่ในแชท **การเรียนรู้เป็นอัตโนมัติ**: ทุก workflow ที่เสร็จสมบูรณ์จะ append สัญญาณ failure/loop/reject ของตัวเองไปยัง `.planning/LEARNINGS.md` ซึ่งจะถูก inject เข้าสู่รอบถัดไป — เป็น always-on **ไม่** ถูก gate ด้วย Retro ที่เป็น optional ส่วน Retro (`/retro`) คือสรุป milestone แยกต่างหากแบบ optional
...
## 🧱 three-layer stack คืออะไร?

three-layer stack ของ harnessed คือการ implement แบบ software-engineering ของการซ้อน **BDD → SDD → TDD** ที่เป็นที่ยอมรับอยู่แล้ว: feedback loop ที่ซ้อนกันสามชั้น แต่ละชั้นตอบคำถามที่ต่างกัน **สามชั้นคือลูป** (ทฤษฎีที่เสถียร); harnessed **composes** ระบบนิเวศโอเพ่นซอร์สเข้าสู่แต่ละลูป — และ component เหล่านั้น **ทับซ้อนกัน** ซึ่งเป็นสิ่งที่ composition orchestrator ทำหน้าที่ตัดสิน (arbitrate) พอดี

| Layer | Loop | คำถามที่ตอบ | ประกอบจาก (ทับซ้อนกัน) |
|---|---|---|---|
...
- **Forward continuation** — `harnessed next` / `harnessed advance` พาคุณข้าม task และ phase: เมื่อตัวหนึ่งเสร็จ ตัวถัดไปจะถูก **derive จาก disk state ของ `.planning/`** (phase เสร็จเมื่อ `PLAN` ของมันมี `SUMMARY` ที่ตรงกัน) — ไม่ต้องดูแล queue ใด ๆ ดังนั้น phase ใหม่ที่แทรกกลางทางจะถูกหยิบมาทำอัตโนมัติ และการ resume จะ re-derive จากดิสก์ มี breadcrumb `NEXT-UNIT` ต่อ turn ชี้ว่าอะไรคือสิ่งถัดไป

---

## 🆚 เทียบกับ Native Claude Code / Codex

Native agent ให้ primitive แก่คุณ; harnessed ร้อยเรียงมันเข้าเป็นเมธดวิทยา ในจุดที่ช่อง native บอกว่า primitive "มีอยู่" คุณยังต้องออกแบบ ร้อยสาย และดูแลมันเองในแต่ละโปรเจกต์ — harnessed ส่งมอบมันแบบ pre-composed และขับเคลื่อนด้วย engine
...
**ตัวติดตั้งบรรทัดเดียว (ไม่ต้องมี Node.js)** — ไบนารีแบบสแตนด์อโลน อัปเดตตัวเองผ่าน `harnessed update`:
...
**หรือผ่าน npm** (ทั้งสองช่องทางเป็น first-class และซิงก์เวอร์ชันกัน):
...
```bash
# 1. Install (เลือกช่องทางใดก็ได้ — ดู Quick Install ด้านบน)
npm install -g harnessed && harnessed setup
# หรือ binary (ไม่ต้องมี Node.js): curl -fsSL https://raw.githubusercontent.com/easyinplay/harnessed/main/install.sh | bash && harnessed setup
```
...
| `/verify-eval-review` | ④ Verify | Sub | GSD `/gsd-eval-review` | ตรวจสอบ coverage ของ AI phase eval (has_ai_phase conditional; จับคู่กับ gsd-ai-integration-phase ฝั่ง plan) |
| `/verify-validate-phase` | ④ Verify | Sub | GSD `/gsd-validate-phase` | เติมช่องโหว่ coverage แบบ Nyquist requirement→test (requires_coverage_audit conditional) |
...
| `/ship-preflight` | ⑤ Ship | Sub | `harnessed release-preflight` | Release-readiness gate แบบ read-only (CHANGELOG `[Unreleased]` / version / git-clean / tag-absent); block เมื่อ fail |
...
เมธดวิทยา 5-stage three-layer-stack — แนะนำให้ขับเคลื่อนผ่าน 5 master orchestrators ตามลำดับ:
...
# 2. รัน 5-stage cadence ภายใน Claude Code
...
├── workflows/                  # L6: composition skills (ไม้บาตอง 5-stage)
...
| `harnessed` (no args) | You-are-here แบบ zero-arg: dashboard ของ active-workflow + `NEXT: auto\|manual\|done` + run hint; `--json` แบบ machine-readable; ไม่มี active workflow → onboarding hint (comet `/comet` analog, read-only) |
| `harnessed next` | สัญญา (contract) ของขั้นถัดไปแบบ deterministic ภายใน workflow: `NEXT: auto\|manual\|done` เมื่อ subs ของ workflow resolve ครบ จะหล่นไปยัง **cross-unit** ถัดไป (phase/task ถัดไปที่ derive จาก disk state ของ `.planning/`) พร้อม exit-code contract (`0` advance · `2` done · `10` blocked) |
| `harnessed advance` | Forward continuation — พิมพ์ work unit ถัดไป (phase/task ถัดไป) ข้าม milestone และคำสั่งสำหรับรันมัน Print-only (main session เป็นผู้รัน `/auto` ถัดไป); ปฏิเสธการก้าวข้าม phase ก่อนหน้าที่ยังไม่เสร็จ (`--force` override ได้); `--json` ขับลูป `while harnessed advance --json; do :; done` |
| `harnessed reject <sub>` | ทำเครื่องหมาย sub ว่าถูก user-rejected (terminal, ต่างจาก `failed`) |
| `harnessed compact [--tokens <n>]` | Summarize+evict ledger entries ที่ resolve แล้ว (G6-safe: `fail_count>0` ไม่ถูก evict); auto-trigger เมื่อ `checkpoint complete --tokens` |
| `harnessed workflows` | แสดง workflows ที่กำลังดำเนินอยู่ (หนึ่งตัวต่อ repo) |
| `harnessed learn "<lesson>"` | Append บทเรียนแบบ prose ไปยัง `.planning/LEARNINGS.md` ของ repo นี้ |
...
```bash
harnessed setup  # ติดตั้ง gstack + GSD + superpowers + planning-with-files อัตโนมัติ; 28 workflow skills ลงใน ~/.claude/skills/ + Agent Teams env var เขียนอัตโนมัติใน ~/.claude/settings.json
```
...
`/discuss /plan /task /verify /ship` — 5 masters ร้อยเรียง 5 stage เข้าด้วยกัน; แต่ละ master delegate ไปยัง sub ของตัวเอง แต่ละ stage ทำสิ่งที่ต่างกันและส่งต่อไปยัง stage ถัดไป **ไม่มีการรวมกัน**
...
Output ของแต่ละ phase ถูกเขียนไปที่ `~/.claude/harnessed/checkpoints/`; หลัง session ถูกขัดจังหวะ `harnessed resume` ดำเนินการต่อจาก checkpoint ล่าสุด
...
- `npx harnessed@latest setup` รัน **Node.js CLI** (`bin/harnessed`) — หรือใช้ไบนารีสแตนด์อโลนจากตัวติดตั้งบรรทัดเดียว (ไม่ต้องมี Node.js)
...
- CLI และ CC skills ใช้ state directory `~/.claude/harnessed/checkpoints/` ร่วมกัน
