...
- **Composition Skill** — workflow skills ภายในทำหน้าที่เหมือนไม้บาตองของวาทยกร ประสาน upstream หลายตัวให้บรรเลงพร้อมกัน **1 super-master `/auto` + 5 stage masters + 19 sub-workflows + 2 standalones = 27 workflows แบบ namespace-layered** machine-execute ครบ 5 stage (`/auto` one-shot ข้ามทุก stage / `/discuss /plan /task /verify /ship` ทีละ stage / 19 three-layer-stack subs / `/research /retro` 2 standalones)
...
# ... หรือเลือก sub-workflow อื่น ๆ จาก 19 ตัว
```
...
| `harnessed doctor` | Health check 14 รายการ (Node / MCP / jq / Win bash / routing / token budget / mattpocock / CodeGraph / update-available ฯลฯ) |
| `harnessed update [--check\|--upstreams\|--migration-report]` | Self-update (`npm i -g harnessed@latest`); `--check` รายงานเวอร์ชันล่าสุด; `--upstreams` re-run base manifests; `--migration-report` คือ inventory ของ stale-state แบบ read-only |
...
```bash
harnessed setup  # ติดตั้ง gstack + GSD + superpowers + planning-with-files อัตโนมัติ; 26 workflow skills ลงใน ~/.claude/skills/ + Agent Teams env var เขียนอัตโนมัติใน ~/.claude.json
```
