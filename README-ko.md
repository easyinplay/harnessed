...
**Claude Code에서 엔지니어링을 위한 모범 사례 Orchestration** — 최고의 오픈소스 Claude Code 생태계 컴포넌트를 조합하여, 독자적인 composition skill을 통해 통합 Workflow로 엮어 냅니다. 상위 코드를 vendor하지 않으며 — manifest가 install/check를 기술하고, composition skill이 다중 상위 협업을 orchestrate합니다.
...
- **3계층 스택 기계 실행** — `gstack governance` + `GSD project manager` + `superpowers senior engineer` + `karpathy 4 원칙` + `mattpocock 23 기법`, 5개 축 100% 커버
- **상위 코드 vendoring 없음** — manifest가 install/check를 기술하므로, 상위 업그레이드 시 재설치만 하면 최신 버전 획득
- **Composition Skill** — 자체 Workflow skill이 지휘봉 역할을 하며 다수의 상위를 협주합니다. **1개 슈퍼 마스터 `/auto` + 4개 Stage 마스터 + 18개 서브 Workflow + 2개 독립형 = 25개 네임스페이스 계층 Workflow**, 완전한 4-Stage 기계 실행(`/auto` 전 Stage 원샷 / `/discuss /plan /task /verify` 단일 Stage / 18개 3계층 스택 서브 / `/research /retro` 2개 독립형)
- **L0 Discipline Substrate** — 전역 Cross-Stage 행동 기준선(karpathy 원칙 + output-style + language + operational + priority + protocols), 모든 Stage에 적용
- **패키지 매니저 마인드셋** — install 의존성 그래프 자동 해석, doctor 건강 체크, install-base 원샷 전체 설치
- **통합 진입점** — 사용자는 각 상위의 용어를 배우지 않고 `/discuss /plan /task /verify` 마스터 slash command만 사용하면 됩니다. 서브 명령어는 단일 Stage를 명시적으로 호출합니다(예: `/discuss-strategic`은 전략 계층 명료화만 실행)
...
**harnessed 제거:**
```bash
harnessed uninstall    # harnessed 자체 파일만 제거 (상위 컴포넌트는 영향 없음)
```

> `harnessed uninstall`은 commands, workflow skills, settings 환경 변수, 상태 디렉터리를 정리합니다. 상위 컴포넌트(npm 패키지, MCP 서버, CC 플러그인, git clone 저장소, npx skills)는 그대로 유지됩니다. `harnessed uninstall <name>`으로 단일 상위 컴포넌트를 제거할 수 있습니다. `--dry-run`으로 미리 볼 수 있습니다.
...
/verify "phase-1"              # 7개 서브 조건부 검증
```

> 어느 Stage부터 시작할지 결정하거나 중간 출력을 검토하고 싶은 경우 — 4개 마스터를 독립적으로 호출할 수 있으며, 각 마스터는 해당 Stage의 모든 서브를 내부에서 자동 팬아웃합니다.
...
# ... 나머지 18개 서브 Workflow 중 원하는 것 선택
```
...
> 점선 박스 = 선택적 독립 실행형(`/research` 전략 전 조사 / `/retro` 마일스톤 후 요약). 실선 박스 = 메인 4-Stage 케이던스.
...
| `/verify` | ④ Verify | 마스터 | masterOrchestrator | 시나리오별 조건부 디스패치로 7개 서브 실행 |
...
| `/verify-design` | ④ Verify | 서브 | gstack `/design-review` + ui-ux-pro-max + frontend-design | 디자인 시스템 일관성 (has_design_changes 조건부) |
...
4-Stage 3계층 스택 방법론 — 4개 마스터 orchestrator를 직렬로 구동하는 것을 권장합니다:
...
# 2. Claude Code 내에서 4-Stage 케이던스 실행
...
├── workflows/                  # L6: composition skill (4-Stage 지휘봉)
...
│   /discuss /plan /task /verify (마스터) + 18 서브 + /research /retro + /auto 슈퍼 마스터
...
| UI 디자인 주요 계획 | ui-ux-pro-max → frontend-design (사용자가 스타일을 명시적으로 요청) |
...
| `harnessed run <name>` | workflow 실행 (master orchestrator 또는 sub). slash command는 이 서브 커맨드를 통해 호출됨 |
| `harnessed resume` | 세션 중단 후 가장 최근 체크포인트에서 재개 |
| `harnessed status` | 현재 Phase + 잠금 보유자 |
| `harnessed doctor` | 8항목 건강 체크 (Node / MCP / jq / Win bash / routing / token budget 등) |
| `harnessed install <name>` | Upstream manifest 설치 |
| `harnessed uninstall [name]` | 통합 제거 — 이름 미지정: harnessed 자체 파일 제거 (상위 컴포넌트 보존); 이름 지정: 단일 상위 컴포넌트 제거 |
...
| `--task <text>` | `run` 서브 커맨드 — 태스크 설명 (workflow `gateContext.task`에 전달됨) |
| `--task-stdin` | `run` 서브 커맨드 — stdin에서 EOF까지 태스크 설명 읽기 (따옴표/$/` shell-escape 회피) |
...
```bash
harnessed setup  # gstack + GSD + superpowers + planning-with-files 자동 설치; 25개 Workflow skill이 ~/.claude/skills/에 배치 + Agent Teams 환경 변수가 ~/.claude.json에 자동 기록
```
...
`/discuss /plan /task /verify` — 4개 마스터가 4개 Stage를 연결합니다. 각 마스터는 내부에서 서브에 위임합니다. 각 Stage는 다른 일을 하고 다음 Stage로 이어집니다. **통합 없음**.
