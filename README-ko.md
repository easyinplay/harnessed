...
- **Composition Skill** — 자체 Workflow skill이 지휘봉 역할을 하며 다수의 상위를 협주합니다. **1개 슈퍼 마스터 `/auto` + 5개 Stage 마스터 + 19개 서브 Workflow + 2개 독립형 = 27개 네임스페이스 계층 Workflow**, 완전한 5-Stage 기계 실행(`/auto` 전 Stage 원샷 / `/discuss /plan /task /verify /ship` 단일 Stage / 19개 3계층 스택 서브 / `/research /retro` 2개 독립형)
...
# ... 나머지 19개 서브 Workflow 중 원하는 것 선택
```
...
│   /discuss /plan /task /verify /ship (마스터) + 19 서브 + /research /retro + /auto 슈퍼 마스터
