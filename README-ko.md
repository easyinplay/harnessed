...
**원라인 설치기 (Node.js 불필요)** — 독립 실행 바이너리, 이후 `harnessed update`로 자가 업데이트:
...
**또는 npm으로** (두 채널 모두 1급 지원, 버전 동기화 유지):
...
```bash
harnessed setup  # gstack + GSD + superpowers + planning-with-files 자동 설치; 28개 Workflow skill이 ~/.claude/skills/에 배치 + Agent Teams 환경 변수가 ~/.claude/settings.json에 자동 기록
```
...
각 Phase 출력은 `~/.claude/harnessed/checkpoints/`에 기록됩니다. 세션 중단 후 `harnessed resume`이 가장 최근 체크포인트에서 재개합니다.
...
- `npx harnessed@latest setup`이 **Node.js CLI** (`bin/harnessed`)를 실행합니다 — 또는 원라인 설치기의 독립 바이너리 사용 (Node.js 불필요)
...
- CLI와 CC skill은 `~/.claude/harnessed/checkpoints/` 상태 디렉터리를 공유합니다
