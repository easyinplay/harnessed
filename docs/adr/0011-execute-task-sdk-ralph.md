# ADR-0011: execute-task workflow + ralph-loop SDK introduction + dual-signal completion + per-phase model tier + Wave 0 transparency gate flip (phase 2.2 — 6 章节 errata 合并)

## Status

**Draft** — 2026-05-15 (phase 2.2 plan-phase Wave 0 sketch；Wave 6 fill-out + flip to **Accepted** at phase 2.2 ship)

> Phase 2.2 execute-task workflow + ralph-loop full integration 的合并 ADR — 沿袭 ADR 0008 / 0009 / 0010 多决策合并 errata 模式（B-20 lock）。本 draft 仅 sketch 6 章节标题与一句 intent；Wave 6 T6.1 由 ship 时 plan-phase planner fill-out 实测细节、引用证据、A7 守恒命令。
...
本 ADR 覆盖 Phase 2.2 6 大决策线，合并入单 ADR（B-20）— 沿袭 phase 1.4 ADR 0008 多决策合并 errata 模式。ADR 编号 `0011` 由 phase 2.2 plan-phase Wave 0 实占（intel § 0 SSOT 引用纪律 + Phase 2.1 T1.9 CONTRIBUTING.md 项目级规则）— Phase 2.1 D-08 暂记 `0011` 仅参考，本 plan-phase 实占值与之一致。
...
- `.planning/phase-2.2/2.2-CONTEXT.md` D-01~D-15（discuss-phase 锁）
...
- `.planning/phase-2.2/ASSUMPTIONS.md` § B 31 lock（合并 CONTEXT 15 + PATTERNS 7 + RESEARCH 7）
- `.planning/phase-2.2/PLAN.md` Wave 0-6 + § 6 F1-F8 reproduction commands
- `.planning/phase-2.2/task_plan.md` T0.1~T6.7 atomic tasks

### 外部参考

- `@anthropic-ai/claude-agent-sdk` (`/nothflare/claude-agent-sdk-docs` ctx7 821 snippets, 2026-05-15) — `outputFormat: { type: 'json_schema' }` structured-outputs.md + `agents` map subagents.md + `AgentDefinition` 4-field type + `SDKResultMessage.structured_output`
