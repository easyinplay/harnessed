...
**Accepted (2026-06-17)** — gstack `/plan-eng-review`(Finding 1-5 absorbed)+ ECC skill inventory 实测枚举(246 skills / 61 agents,§ 枚举结论 partition 核对 + 3 处修正 absorbed)均已完成。de-confliction 矩阵锁定。剩余 wire 前门:`harnessed manifest-add` EE-5 gate。本 ADR 是动 `manifests/optional/ecc.yaml` + `workflows/capabilities.yaml` 之前的 de-confliction SoT。
...
`manifests/optional/ecc.yaml`(**opt-in 放 optional/** — setup/install-base 只 glob tools+skill-packs,故 ECC 不被自动装;sister codegraph),`spec.install.method` 走 plugin/marketplace(`ecc`),`idempotent_check` + `verify` 照 `gstack.yaml` 模板。不 vendor ECC 代码。ECC 是 optional 上游 + pre-release,用户显式 opt-in(`claude plugin install ecc@ecc`)。
...
- `manifests/optional/ecc.yaml`(**optional/ opt-in** — setup 不自动装;cc-plugin-marketplace,git_ref ec92b52,stability unstable+warn)+ `ecc.ee5-answers.json`(EE-5 PASSED)。
...
### E-1 (2026-06-17) — manifest placement: skill-packs/ → optional/ (setup auto-install fix)

**Surface**: post-wire dogfood question「setup 兼容 ECC 了吗」caught a real bug. The manifest was first written to `manifests/skill-packs/ecc.yaml`, but `harnessed setup` Step B auto-globs `manifests/{tools,skill-packs}/*.yaml` (src/cli/setup.ts:45) — so setup would have **auto-installed ECC (a 2.0.0-rc.1 pre-release) for every user**, contradicting the optional-upstream + opt-in design.

**Resolution**: `git mv` the manifest + EE-5 record to `manifests/optional/` (sister codegraph — optional/ is NOT globbed by setup/install-base). ECC now opt-in only; the capabilities.yaml Bucket 11 entries were already opt-in (no presence-check, no-op when ECC absent), so no routing change needed.

**Follow-up RESOLVED (2026-06-23, E-2)**: `harnessed install ecc` now resolves `optional/`. See Errata E-2.

### E-2 (2026-06-23) — `harnessed install <name>` third resolution tier: optional/

**Surface**: E-1 left `harnessed install ecc` returning `manifest not found` — ECC could only be installed via the raw `claude plugin install ecc@ecc` cmd. The opt-in catalog (`optional/`) was reachable by neither setup (by design) nor explicit install (a gap).

**Resolution**: extended `src/cli/install.ts` manifest resolution from two tiers to three — `tools → skill-packs → optional`. The `optional/` tier stays out of `harnessed setup` Step B globbing (still no auto-install; `setup.ts:45` unchanged) but is now reachable via **explicit** `harnessed install <name>`. This matches the intended semantics: `optional/` = on-demand, not auto.

**Confirmed end-to-end**: `harnessed install ecc --dry-run` resolves `manifests/optional/ecc.yaml`, dispatches the cc-plugin-marketplace installer, renders the diff (enabledPlugins `ecc@ecc`), exits 2 (dry-run user-cancel — i.e. resolved, not exit-1-not-found). Sister `harnessed install codegraph` likewise resolves (npm-cli method). The `install.manifest_not_found.fix` hint (en + zh-Hans) now lists all three tiers. `codegraph.yaml` header comment updated (its prior "harnessed install never installs it" claim was made false by this change). Regression coverage: `tests/unit/cli-install.test.ts` "optional tier" case asserts the three-tier order + that `optional/` is the resolving tier.
