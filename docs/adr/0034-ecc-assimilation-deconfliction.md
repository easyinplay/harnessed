...
4. `harnessed manifest-add` EE-5 gate(5 问)— **下一步**
5. 再 wire:`ecc.yaml` manifest → capabilities.yaml Bucket 11 + verify alias → judgments/*.yaml fires_when/skips_when → verify
...
**Proposed — 未 wire。** 本 ADR 是 de-confliction SoT 草案;capabilities.yaml + ecc.yaml manifest 在 plan-eng-review + EE-5 gate + 本 ADR Accepted 之后才动。A posture 先行,B 升级逐个走 errata ADR。

## Errata

(none yet — Proposed 状态)
