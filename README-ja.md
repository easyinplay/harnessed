...
- **Composition Skill** —— 自社製 workflow skill が指揮棒として機能し、複数の上流を協調させる。**1 つのスーパーマスター `/auto` + 5 つのステージマスター + 20 サブ workflow + 2 スタンドアロン = 28 の名前空間階層型 workflow**、5 ステージを完全機械実行（`/auto` でステージ横断ワンショット / `/discuss /plan /task /verify /ship` で単一ステージ / 20 の三層スタックサブ / `/research /retro` の 2 スタンドアロン）
...
**ワンライナーインストーラー(Node.js 不要)** —— スタンドアロンバイナリ。以後は `harnessed update` で自己更新:
...
**または npm 経由**(両チャネルとも第一級サポート、バージョンは同期):
...
```bash
# 1. インストール（どちらのチャネルでも可 — 上の Quick Install 参照）
npm install -g harnessed && harnessed setup
# またはバイナリ（Node.js 不要）: curl -fsSL https://raw.githubusercontent.com/easyinplay/harnessed/main/install.sh | bash && harnessed setup
```
...
| `/verify-eval-review` | ④ Verify | サブ | GSD `/gsd-eval-review` | AI phase eval カバレッジ監査（has_ai_phase 条件付き；plan 側の gsd-ai-integration-phase と対） |
| `/verify-validate-phase` | ④ Verify | サブ | GSD `/gsd-validate-phase` | Nyquist requirement→test カバレッジ補完（requires_coverage_audit 条件付き） |
...
│   /discuss /plan /task /verify /ship（マスター）+ 20 サブ + /research /retro + /auto スーパーマスター
...
```bash
harnessed setup  # gstack + GSD + superpowers + planning-with-files を自動インストール。28 の workflow skill が ~/.claude/skills/ に配置 + Agent Teams 環境変数が ~/.claude/settings.json に自動書き込み
```
...
各フェーズの出力は `~/.claude/harnessed/checkpoints/` に書き込まれます。セッション中断後は `harnessed resume` で最新チェックポイントから再開します。
...
- `npx harnessed@latest setup` は **Node.js CLI**（`bin/harnessed`）を実行します — またはワンライナーインストーラーのスタンドアロンバイナリを使用(Node.js 不要)
...
- CLI と CC Skill は `~/.claude/harnessed/checkpoints/` 状態ディレクトリを共有します
