...
- **Composition Skill** —— 自社製 workflow skill が指揮棒として機能し、複数の上流を協調させる。**1 つのスーパーマスター `/auto` + 5 つのステージマスター + 21 サブ workflow + 2 スタンドアロン = 29 の名前空間階層型 workflow**、5 ステージを完全機械実行（`/auto` でステージ横断ワンショット / `/discuss /plan /task /verify /ship` で単一ステージ / 21 の三層スタックサブ / `/research /retro` の 2 スタンドアロン）
...
| `/verify` | ④ Verify | マスター | masterOrchestrator | シナリオ別に 9 サブを条件付きディスパッチ |
...
│   /discuss /plan /task /verify /ship（マスター）+ 21 サブ + /research /retro + /auto スーパーマスター
