...
function buildOrchestratorBody(name: string, prompt: RolePrompt): string {
  ...
  return [
    ...
    ``,
    ...autoDiscussStep,
    // T2.1 — MUST stay in lockstep with `factsStepEn` in
    // scripts/rewrite-skill-invoke-sections.mjs (copied verbatim; the SKILL is
    // the sister rendering of this same sequence). Without this step the gates
    // below evaluate against generic defaults, which is the audit-S2 defect
    // itself: every judgement-call fact pinned to its firing side and no caller
    // ever passing context, i.e. criteria with zero discriminating power.
