...
function buildOrchestratorBody(name: string, prompt: RolePrompt): string {
  ...
  return [
    ...
    ``,
    ...autoDiscussStep,
    `2. Bash: \`harnessed gates ${name} --task "<locked spec>" --skip-sub discuss\` → parse the JSON \`{fire: [{sub, order, mode, is_master}], skip: [{sub, reason}], parallelism: {escalate_to_teams}}\`. This is the plan SoT (no spawn). Keep the verbatim JSON for the next step.${name === 'auto' ? ' For a small self-contained task (single-file / single-page class), the sanctioned lite path is adding `--skip-sub verify --skip-sub retro` (repeatable / comma-separated) — skipped subs are still recorded in the ledger with reasons; lite ≠ freestyle (the ledger/evidence IS the difference).' : ''}`,
    ...
    `- The per-sub ledger + evidence guard live in \`workflows.json\` (per-repo store, sole SoT) — written only by \`harnessed checkpoint\`; never edit state files directly.`,
    ``,
    MARKER,
    ``,
  ].join('\n')
