...
export function getPackageRoot(): string {
  // import.meta.url → file:///.../dist/cli.mjs (or src/platform/packagePath.ts in test)
  // Go up: dist/cli.mjs → dist/ → package root  (1 level from dist/)
  //        src/platform/packagePath.ts → src/platform/ → src/ → package root (2 levels in source)
  // We detect which context we're in by checking for the 'dist' segment.
  const thisFile = fileURLToPath(import.meta.url)
  const thisDir = dirname(thisFile)
  // In dist: thisDir = <root>/dist        → up 1
  // In src:  thisDir = <root>/src/platform → up 2 (#7 move: was src/cli/lib → up 3)
  if (thisDir.endsWith('dist') || thisDir.replace(/\\/g, '/').endsWith('/dist')) {
    return resolve(thisDir, '..')
  }
  // Source / test context — go up 2
  return resolve(thisDir, '..', '..')
}
