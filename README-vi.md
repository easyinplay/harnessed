...
- **Composition Skill** — workflow skill nội bộ đóng vai trò cây đũa chỉ huy, điều phối nhiều upstream hoạt động cùng nhau. **1 super-master `/auto` + 5 stage masters + 20 sub-workflows + 2 standalones = 28 workflow phân tầng namespace**, thực thi máy đầy đủ 5-stage (`/auto` one-shot qua các stage / `/discuss /plan /task /verify /ship` từng stage / 20 three-layer-stack subs / `/research /retro` 2 standalones)
...
/verify "phase-1"             # 10 sub verification có điều kiện
```
...
# ... chọn bất kỳ sub-workflow nào trong số 20 cái còn lại
```
...
/verify "phase-1"                 # 10 sub có điều kiện
