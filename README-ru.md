...
**Однострочный установщик (Node.js не требуется)** — автономный бинарник, далее самообновляется через `harnessed update`:
...
**Или через npm** (оба канала полноценны и синхронизированы):
...
```bash
# 1. Установка (любой канал — см. Quick Install выше)
npm install -g harnessed && harnessed setup
# или бинарник (без Node.js): curl -fsSL https://raw.githubusercontent.com/easyinplay/harnessed/main/install.sh | bash && harnessed setup
```
