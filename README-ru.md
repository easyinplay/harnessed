...
- **Composition Skill** — собственные workflow-скиллы играют роль дирижёрской палочки, оркестрируя несколько апстримов в унисон. **1 супер-мастер `/auto` + 5 мастеров стадий + 21 суб-воркфлоу + 2 standalone = 29 workflow с неймспейс-иерархией**, полное 5-стадийное машинное исполнение (`/auto` одним выстрелом по всем стадиям / `/discuss /plan /task /verify /ship` по одной стадии / 21 суб-воркфлоу three-layer-stack / `/research /retro` 2 standalone)
...
```bash
# 1. Установка (одна строка)
npm install -g harnessed && harnessed setup
```

```
# 2. Внутри Claude Code — запустите первый workflow
/auto "ваше первое требование"        # дефолт для новичка: прогоняет все стадии end-to-end
```

```bash
# 3. Потерялись? Запустите harnessed без аргументов — он скажет, где вы и что дальше
harnessed
...
```bash
# 4. Возобновляйте в любой момент после прерывания
...
/verify "фаза-1"                 # 9 суб-воркфлоу условно
```
...
# ... выберите любой из остальных 21 суб-воркфлоу
```
...
| `/verify` | ④ Verify | Мастер | masterOrchestrator | 9 суб-воркфлоу с условной диспетчеризацией по сценарию |
...
│   /discuss /plan /task /verify /ship (мастер) + 21 суб + /research /retro + /auto супер-мастер
...
| `harnessed doctor` | Health check с 14 проверками (Node / MCP / jq / Win bash / routing / token budget / mattpocock / CodeGraph / update-available и т.д.) |
| `harnessed update [--check\|--upstreams\|--migration-report]` | Самообновление (`npm i -g harnessed@latest`); `--check` сообщает последнюю версию; `--upstreams` перезапускает базовые manifest-ы; `--migration-report` — read-only инвентаризация устаревшего состояния |
...
```bash
harnessed setup  # Автоматически устанавливает gstack + GSD + superpowers + planning-with-files; 26 workflow-скиллов попадают в ~/.claude/skills/ + переменная окружения Agent Teams автоматически записывается в ~/.claude.json
```
