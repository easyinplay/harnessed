...
| 31 ✅ | SKILL.md surface translation | ✅ Done 2026-06-24 — **26** `SKILL.zh-Hans.md` siblings (~10,132 words) via 7 parallel translation subagents; guard exit 0 (structural parity), en byte-identical, vitest 1423/0. Prose (TDD-skip). yaml split → Phase 33. 详: `phases/31-skill-translation/31-01-SUMMARY.md`. |
| 32 ✅ | CLI message table gap close | ✅ Done 2026-06-24 — `messages/zh-Hans.json` 80→94 full key parity with en.json + `i18n-parity.test.ts` (both-direction). 16 `uninstall.unified.*` added, 2 dead keys removed (count 14→16 reconciled). vitest 1426/0. 详: `phases/32-cli-message-gap/32-01-SUMMARY.md`. |
| 33 ✅ | user-facing yaml i18n | ✅ Done 2026-06-24 — `resolveLocaleYaml` loader + `role-prompts.zh-Hans.yaml` (24 roles) + 5 discipline zh siblings; **fixed pre-existing en-default bug** (en users now get English disciplines, 4 Chinese-source bases→English + zh siblings byte-identical to HEAD). `language` excluded (never surfaced). Guard `check-yaml-i18n-parity.mjs` + ci. vitest 1446/0 (+20). 详: `phases/33-yaml-i18n/33-01-SUMMARY.md`. |
