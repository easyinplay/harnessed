...
| v12.0 Forward Continuation | 2026-06-30 (npm 4.10.0) | Cross-task/phase advance (closes the horizontal-续作 gap): `deriveNext` + `planningScan` (derive next from `.planning` disk SoT, **no queue**; phase complete ⇔ PLAN has matching SUMMARY, artifact-derived) + `harnessed next` cross-unit exit-codes (0 advance/2 done/10 blocked) + `harnessed advance` (print-only + advance-gate, `--json` driver loop) + per-turn `NEXT-UNIT` pointer (ts↔bin parity). 5-repo fusion (gsd-pi/gsd-core/Trellis/comet/oh-my-pi; derive-not-queue). Bundled the issue #2 fix (52 SKILL invoke sections inline the state-machine + `ship` promoted to orchestrator). Phase-level floor (OQ-2); task-level resolver-ready. 1673 tests. Published npm `harnessed@4.10.0` (provenance) + GitHub release `v4.10.0`. 详: `phases/37-derive-next/`…`phases/40-docs-verify/` SUMMARYs + spec `specs/2026-06-30-forward-continuation-design.md`. |
| v13.0 Upstream Re-sync | 2026-07-01 (npm 4.12.0) | 4 phases (41–44): gstack 1.52.1.0→1.58.0.0 + gsd-core ^1.4.1→^1.6.0 + superpowers v5.1.0→v6.0.3 pin bump (继承上游硬化; 窗口 0 新 user-facing skill — 白拿 token carving/安全 guard/UAT routing) + 5 selective wire (verification-before-completion task 证据门 · gsd-eval-review + gsd-validate-phase verify conditional sub · /diagram gstack B7 · systematic-debugging gsd-debug alias)。capabilities 35→39, verify sub 5→7。3-subagent delta 调研 + 2 spike (gsd 安装面/superpowers worktree 兼容); skip MemPalace(记忆双 home)/gbrain/ios (ADR-0035 D3)。Published npm `harnessed@4.12.0` (provenance) + GitHub release `v4.12.0`, CI 3-OS green. 详: ADR-0035 + spec `specs/2026-06-30-upstream-resync-v13.md` (phase dirs 未建, spec 统领 + 主 session 手控). |
