...
> **Resolved (T0.1 STATE trim outcome, PENDING)**: <PENDING — Wave 0 T0.1 fill: `wc -l .planning/STATE.md` post-trim 实测 (expect ≤200L round 1; if larger → escalate D1 trim was insufficient, consider extending archive to L448-517 Session 连续性 70L OR L386-447 Decisions section flatten 50L per RESEARCH § 5.1); `node scripts/check-state-archive-stale.mjs` exit 0 (warn-only round 1, may print violations but exit 0)>

> **Resolved (T0.2 install.ts Path A vs B outcome, PENDING)**: <PENDING — Wave 0 T0.2 fill: Path A `import pkg from '../../package.json' with { type: 'json' }` typecheck outcome (`pnpm typecheck 2>&1 | tail -3` — expect exit 0; if blocked by tsup/TS pipeline `with` syntax → revert to Path B readFileSync fallback per RESEARCH § 6.2 A1 ASSUMED LOW risk); `wc -l src/cli/install.ts` post-modify (expect 140 → 141L Path A / 144L Path B; both ≤200)>

> **Resolved (T0.3 versions/0.3.0-known-good.yaml fill outcome, PENDING)**: <PENDING — Wave 0 T0.3 fill: 8 entries actual install_method values (planner cross-ref `manifests/{skill-packs/superpowers,planning-with-files,mattpocock-skills}.yaml` for exact install_method per RESEARCH § 7.1 candidate list); `pnpm test -- --run tests/manifest/knownGood.test.ts 2>&1 | tail -3` Value.Check pass outcome (expect 5 fixture pass post-fill)>
...
> **Resolved (T0.5 SAMPLES.md mining outcome, PENDING)**: <PENDING — Wave 0 T0.5 fill: actual 30 sample selection per-tier (10 Haiku + 10 Sonnet + 10 Opus) — executor surface task_id list (T01-T30) + source_commit shas + expected_decision yaml inline per row; `wc -l .planning/phase-3.4/SAMPLES.md` outcome (expect 30 row × 6L avg = ~180L ≤200L Karpathy hard); per-row source_commit field non-empty verify (D-01 sneak block守门)>
