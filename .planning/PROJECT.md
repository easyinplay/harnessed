...
## Current Milestone: (none active) — v8.0 Frictionless Entry shipped 2026-06-23

> Charter era-pointer only; live per-milestone position lives in `STATE.md`. (This section was stale at v5.1 through v6.0/v7.0/v8.0 closes — refreshed at v8.0 close.)

**Last shipped:** **v8.0 Frictionless Entry** (2026-06-23) — single-command resume entry (zero-arg `harnessed` you-are-here dashboard + `NEXT` + `--json`, comet `/comet` analog) + value-prop/quickstart legibility (结果导向定位主句 + First 5 Minutes quickstart). Audit passed 2/2 (`milestones/v8.0-MILESTONE-AUDIT.md`). npm latest `4.6.0`.

**Next:** **v9.0 Cross-Harness** — abstract harnessed's hardcoded `~/.claude/` → platform descriptor + auto-detect installed harness + `.agents/` per-platform convention; opens with `/plan-eng-review`. Then **v10.0 i18n Surface**. (3-milestone split from the 2026-06-23 comet/Trellis re-comparison; posture 守宽做深.)
