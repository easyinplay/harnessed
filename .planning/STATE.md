...
> Status: v0.1.0 SHIPPED & ARCHIVED · v0.2.0 MILESTONE 3/4 ACTIVE · **Phase 2.3 SHIPPED** (2026-05-16) · **Phase 2.4 Wave 0 SHIPPED** (2026-05-16 — 6 atomic commits T0.1-T0.6 + 1 pre-flight tooling commit 3fc0c42 dashboard polish round 2; CI run 25954285185 3-OS green) (sister review N/N: 17/17 freshness gate scope-extended FRONT_MATTER_DOCS 含 STATE.md; miss: 0)
> 最后更新：2026-05-16（**Phase 2.3 SHIPPED** 2026-05-16 — 6 Waves 全 ship 31 atomic commits across W0-W5 main + W6 final ship；详 L24-25 当前位置块。 前置：**Phase 2.2 SHIPPED** 2026-05-15 — execute-task workflow + ralph-loop full SDK integration；6 Waves (W0+W1+W2+W3+W4+W5+W6) 全 ship；SDK `@anthropic-ai/claude-agent-sdk@0.3.142` INTRODUCE NOW + AgentDefinition 5-字段 SDK input + 9-字段 prompt-inject contract v1.2 reconcile；dual-signal completion 4-layer (L1 outer PRIMARY structured_output + L2 outer FALLBACK `<promise>` + L3 inner FALLBACK + L4 retry)；schemaVersion 7-surface 单一兼容门 (CD-5)；per-phase model tier (4 phase × opus/sonnet/sonnet/haiku)；CD-6 provenance gate hard fail BEFORE-W4；CD-4 Task Session 复用 conditional → SC4 PARTIAL → T4.4 DEFERRED → v0.3.0；transparency CI gate W3 ENFORCE=true (Phase 2.1 deferred 根治)；F1-F8 8/8 acceptance bar；ADR 0011 9 章节 errata 合并 (原 6 + discuss-delta 3)；30/30 sample harness COMPLETE 100%；tests 432+3；11 ADR + 11 baseline tag (T6.5 main agent push)。前置 **Phase 2.1 SHIPPED**；前置 **🚀 v0.2.0 MILESTONE ACTIVE** (3/4)；前置 **🎯 v0.1.0 MILESTONE SHIPPED & ARCHIVED**：6/6 phase + audit passed + git tag v0.1.0）
...
- **GSD phase**：✅ **Phase 2.3 SHIPPED**（2026-05-16 — extension category 3 MVP (design/content/testing) + karpathy SKILL-ONLY 注入 + EE-5 5-question CLI gate + CD-3 negative-space arbitrate redirect + 30/30 routing 100% (≥85% bar 远超) + Wave 0 backlog 6 项一次根治；F1-F8 8/8 acceptance bar；ADR 0012 9 章节 errata accepted；12 ADR + 12 baseline tag）— 前置 ✅ **Phase 2.2 SHIPPED** 2026-05-15；前置 ✅ Phase 2.1 SHIPPED；前置 🚀 **v0.2.0 MILESTONE 3/4**；前置 ✅ v0.1.0 SHIPPED & ARCHIVED（6/6 phase；git tag `v0.1.0`）
- **当前里程碑**：v0.2.0 Sub-task Loop + Extension Installers（Phase 2.1-2.4）— **Phase 2.3 3/4 完成**
- **下一 phase**：**Phase 2.4** doctor 完整版 启动待办 — EE-4 plan 4 维量化阈值 schema absorb (omo ⭐⭐) OR phase 2.5 独立；v0.3.0 milestone tag `v0.2.0-alpha.3-extension-mvp` candidate (Wave 6 T6.4 push)
- **状态**：✅ **Phase 2.3 SHIPPED** — Wave 0+1+2+3+4+5+6 全 ship 2026-05-16；A7 守恒 ADR 0001-0012 main body 0 diff（Wave 5 T5.4 mid-wave + Wave 6 T6.4 ship-time 双 verify PASS）；CI 3-OS green；tests 432→492 (+60)；ADR 0012 9 章节 errata accepted；30/30 = 100% routing accuracy (≥85% D-05 acceptance bar 远超 15% headroom)；DI-1 hotfix ship (karpathy-skills.yaml schema-compliant)；T-W6-1 + T-W6-2 + T-W6-3 sister review backlog 全 absorb
- **进度**：9 / 17 phases 已完成 ▓▓▓▓▓▓▓▓▓░░░░░░░░ 52.9%（v0.1.0 里程碑 100%；v0.2.0 3/4 = 75%）
- **Phase 2.3 关键决议 ship 总结**：(1) extension category MVP — 5 NEW manifest (frontend-design + anthropics-skills pptx/slide-deck + playwright-test + chrome-devtools-mcp) 装配 + Wave 1 dry-run e2e；(2) **T0.10 always_active spike FAIL** → R2 A1 fallback locked: description-keyword + self-reflexive prompt SKILL.md body 首段（karpathy 4 心法 always-on via SDK skill discovery，0 frontmatter 字段）；(3) karpathy SKILL-ONLY (D-02) — CLAUDE.md 不动；`harnessed install karpathy-skills` cp -R 干净；(4) CD-3 negative-space (`do_not_use_when` + `if_rejected_use`) + arbitrateWithRedirect 升级 ~15L + per-manifest hint mirror；(5) EE-5 5-question merge gate 双层 — L1 CLI `harnessed manifest-add` 78L + L2 plan-phase 模板 + gsd-plan-checker BLOCKER；(6) 30 FRESH SAMPLES (10/10/10 design/content/testing + 5 anchor + 1 false-pos + 4+ CD-3 disqualify edges) — `tests/routing/samples-30.test.ts` 30/30 = 100% (远超 ≥85%)；(7) **Wave 0 backlog 6 项一次根治** — M1 schema regen drift gate / M2 intel 实施进度回填 / M3 perf gate 移出 CI critical path (D2.3-1 a 终止 50→75→100→130→160 累积 nudge anti-pattern via `perf-bench.yml` nightly cron) / T1.2 schemaVersion consumer call site gate (B1 honest ≥2) / T1.3 Win pwsh provenance sentinel / T5 deferred-items review cadence；(8) **DI-1 hotfix** karpathy-skills.yaml schema-compliant (git_ref 40-hex SHA pin + install_type:git per ADR 0007 1:N closure)；(9) **T-W6-x sister review backlog** — T-W6-1 STATE.md 加入 FRONT_MATTER_DOCS / T-W6-2 dashboard.mjs RETROSPECTIVE 已完成节回写 / T-W6-3 ADR 0012 5→9 章节 fill
...
| v0.2.0 Sub-task Loop + Extension Installers | 3/4 | 🚀 ACTIVE — Phase 2.1 + Phase 2.2 SHIPPED 2026-05-15; **Phase 2.3 SHIPPED 2026-05-16** (6 Waves W0-W6 全 ship 31 atomic commits + DI-1 hotfix + 3 sister backlog) | 启动 2026-05-15 / Phase 2.1 ship 2026-05-15 / Phase 2.2 ship 2026-05-15 / Phase 2.3 ship 2026-05-16 |
| v0.3.0 plan-feature + checkpoint | 0/4 | Not started — T4.4 Task Session 复用 完整版 closure infra ready (3 件套 sdkSpawn.onSessionId / ralphLoopWrap.resumeSessionId / engine.wrappedSpawn capturedSessionId) | - |
...
25. ✅ ~~**Phase 2.4 Wave 0 SHIPPED**~~ (2026-05-16) — 6 atomic commits T0.1-T0.6 (b75b077 / a9ee7ca / 329a4fb / e341048 / 45539a7 / 本 commit) + 1 pre-flight tooling commit (3fc0c42 dashboard polish round 2)；STATE.md L601+ Phase 2.4 Prereq Notes 5 项 backlog 一次根治 (H3+T4 README counter gate B 路径 / M1 dashboard polish round 1 RETRO / M2 schemaVersion 3rd consumer / T2 dashboard absorb 决策 D-04 ship / T3 v0.3.0 prereq 注记) + ADR 0013 9 章节 sketch draft + tests/installers + tests/scripts 目录预创建 + check-deferred-items.mjs cadence 强化；CI run 25954285185 3-OS 全绿 ✅ (ubuntu/macos/windows = success)；Wave 1+ continues — doctor 5 check 完整版 (D-01) / EE-4 plan-checker 4 维 SSOT (D-02) / dashboard C 路径 3 子功能 FULL absorb (D-04) / ralph-loop Win sentinel (F5)
