...
35. ⏳ **v0.3.0 W0 prereq backlog (Phase 3.1 → Phase 3.2 carry-forward)**: DEFERRED #1 (Phase 3.1 W0.3) README completeness check ENFORCE flip → Phase 3.2 W0 prereq after STATE.md/README.md format normalization (sister Phase 2.1 transparency 4-phase warn-only round 1 → ENFORCE pattern); cli-audit 2 pre-existing fail (pre-Phase 1.2) → Phase 3.2 scheduled fix session (out of scope Phase 3.1); DEFERRED #2 userSpawn session_id capture niche → Phase 3.4+ if real demand (fresh-session fallback currently acceptable per B-02)
