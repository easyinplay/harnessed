...
milestone: v3.9.x
milestone_name: v3.9.x Post-v3.0 Maintenance Series (dogfood bug fixes + Windows detection + dead code removal)
status: in_progress
last_updated: "2026-05-27T00:00:00.000Z"
progress:
  total_phases: 33
  completed_phases: 28
  maintenance_releases: 11
...
- **当前关注**：✅ 🎯 **v1.0 GA SHIPPED** 2026-05-22；🎯 **v3.0 GA SHIPPED** 2026-05-21；🔧 **v3.9.x 维护系列 active** — v3.9.17 最新 (dogfood bug fixes + Windows idempotent 检测 + 死代码移除 + code review fixes)；6 milestones (v0.1.0+v0.2.0+v0.3.0+v0.4.0+v0.5.0+v1.0) + v2.0 Architecture Refactor + v3.0 4-Stage Namespace-Layered Workflow；21/21 phases 100% (10 days 2026-05-12~22)；post-v1.0 maintenance-only mode active per ADR 0020 D-04 HYBRID 2-clock
...
- **GSD phase**：🎯 **Phase v3.0-3.6 close + ship SHIPPED — v3.0.0 GA COMPLETE** (2026-05-21)；🔧 **v3.9.x 维护系列 active** — v3.9.26 最新 (2026-05-30, Option A 交互式澄清架构修复)；共 20 个维护版本 (v3.9.7 → v3.9.26)，详见 `.planning/v3.9.x/MAINTENANCE-LOG.md` + `PLAN-interactive-clarification.md`
...
| v3.0 4-Stage Architecture | 6/6 | 🎯 **SHIPPED** — Phase v3.0-v3.6 full 6-phase ship; master orchestrator + 24 sub-workflow + cross-cutting disciplines; execute team Pattern A 3-teammate (13 atomic commit); 1087/1093 pass | 2026-05-21 |
| 🔧 v3.9.x 维护系列 | 11 releases | ✅ **active** — v3.9.7→v3.9.17 (dogfood bug fixes + Windows idempotent 检测 + i18n 统一 + 死代码移除 + code review fixes); 1040/1046 tests pass; 最新 v3.9.17 | 2026-05-24 → 2026-05-27 |
...
- **Phase v2.0-2.6 shipped** 🎯 (2026-05-20) — v2.0 GA final phase

<!-- v3.9.x 维护系列 ship 历史 → 详见 .planning/v3.9.x/MAINTENANCE-LOG.md -->

- **v3.9.17 shipped** 🔧 (2026-05-27) — code review fixes: extractSkillName 去重 (installer + uninstaller 共享 lib) + gate eval phase context 填充 (sub-workflow 直接调用时 gate 不再报 undefined variable) + 死代码 validateNonInteractiveFlags 移除 + 删除 validateFlags.ts + SCHEMA_VERSIONS 注释序号清理 + 10 处过时 source comment 更新 + workflows/README.md & SCHEMA.md 更新 + 3 个测试 mock 从硬编码改为 vi.importActual spread; 1040/1046 tests pass
- **v3.9.16 shipped** 🔧 (2026-05-26) — execute-task/plan-feature/verify-work 彻底移除 (src/cli/execute-task.ts + src/workflow/schema/planFeature.ts + 3 workflow.yaml + FLAT_LEGACY_KEEP 6→3 + SCHEMA_VERSIONS 18→17 + 13 测试文件删除 + defaults.yaml 清理); 1040/1046 tests pass
- **v3.9.7→v3.9.15 shipped** 🔧 (2026-05-24-26) — Windows 原生 idempotent 检测 (isPluginRegistered + fs.access + expandTildeForWindows) + setup 输出精简 (quiet flag + i18n 统一英文) + messages/ npm 包修复 + npm publish race condition 修复 + ctx7 false-negative 修复 + mattpocock 双路径检测; 详见 `.planning/v3.9.x/MAINTENANCE-LOG.md`; W0 6 ADR backfill (0024 workflow schema v2 + 0025 capabilities baseline + 0026 judgments multi-file + 0027 research/verify-work + planning-with-files plugin reframe + 0028 ralph-loop+TDD+Agent Teams+Q-AUDIT-5b schema fix + 0029 fallback 3 铁律+4-stage close) via 6 parallel subagent fan-out (5 SHIPPED + 1 hung→inline write fallback) + 3 deferred items resolved inline (T2.4.W1.4 cascade upgrade + T2.4.W1.5 fallback wire CLI E2E + CK setup.ts split 235→139L) + W1 ship artifacts (CHANGELOG [2.0.0] BREAKING + ci.yml A7 step 0023→0029 + package.json 1.0.4→2.0.0 + README v2.0 4-workflow highlight + ADR README index 6 row append) + W2 milestone close (.planning/milestones/v2.0-MILESTONE-AUDIT.md NEW); 4 LOCAL tag created (v2.0.0-alpha.0-schema + v2.0.0-alpha.1-workflows + v2.0.0-rc.1 + 🎯 v2.0.0 GA); v2.0 Architecture Refactor 6/6 phase 100% COMPLETE; v2.0.0 npm publish pending user push approval per CLAUDE.md commit safety (sister ADR 0023 v1.0 OIDC + sigstore + tag-trigger pattern)
...
[当前: 🔧 v3.9.x 维护系列 active — v3.9.17 (2026-05-27) 最新; dogfood 驱动的 bug 修复 + Windows 兼容 + 代码简化; 每版本发布前必须通过 1040+ tests + biome + tsc; 详见 `.planning/v3.9.x/MAINTENANCE-LOG.md`]
...
1. ✅ **v3.9.7-v3.9.16 SHIPPED** — Windows idempotent 检测 + i18n 统一 + 死代码移除 (execute-task/plan-feature/verify-work)
2. ✅ **v3.9.17 SHIPPED** — code review fixes (extractSkillName dedup + gate eval phase context + 死代码 validateNonInteractiveFlags + comment cleanup)
3. ⏳ **mattpocock-skills install timeout** — `npx skills@latest add mattpocock/skills --copy --global` 在 60s timeout 内无输出, 疑似 skills CLI 交互式 prompt; deferred for future investigation
4. ⏳ **继续 dogfood** — 用户跑 `npm install -g harnessed@latest && harnessed setup` 验证 already-installed 分类
