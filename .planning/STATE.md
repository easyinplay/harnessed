...
29. ✅ ~~**Phase 2.4 ship sister H1+H2 atomic absorb**~~ (2026-05-16, commit `800e459`) — README 6 stale 处一次根治 (L9 Status / L41 heading / L44 milestone / L45 NEW Phase 2.4 entry / L57 NEW acceptance bar / L58 Next; 4th recurrence terminus) + freshness gate 2 sub-bug root-cause 修 (Bug 1: getLatestShippedToken `.match()` 没 `/g` 返回 v0.1.0 oldest not latest → matchAll + last; Bug 2: STATUS_MARKER `.includes()` substring 太弱 "Next: Phase X.Y" 命中误 PASS → milestone 需 SHIPPED/MILESTONE/ARCHIVED 80字符上下文 regex + subphase 需 literal `**Phase X.Y SHIPPED`); pre-fix gate "all compliant" silent PASS, post-fix verify gate 正确 FAIL "missing subphase literal **Phase 2.4 SHIPPED" 捕到; build pass + 543 tests / 4 skipped 与 ship baseline 一致 0 regression; T0.4 freshness gate 第二次结构性硬化 (Phase 2.2 W0 round 1 = 加 STATUS_MARKER; ship 后 round 2 = 修 2 sub-bug); E11 update: freshness gate 实际 catch 二次 (PROJECT-SPEC 61ba0d4 + README 800e459)
