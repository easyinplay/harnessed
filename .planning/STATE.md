...
- **Current focus**: **v11.0 State Machine Completion** opened 2026-06-25 — finish the v5.0 state-machine's deferred Spec 2 + Spec 3 (D session-scoped state / G per-turn injection hook / H scale-adaptive verify strength). Scope SoT = `milestones/v5.0-phases/STATE-MACHINE-CORE-DESIGN.md` §1 (Spec 2/3 lines 43-44). Strategy gate SKIPPED (scope pre-defined in the v5.0 design — transparent). Additive backward-compat. **Phase 34 + 35 DONE; Phase 36 PLANNED (ready-to-execute)** — next action: execute Phase 36 (scale-adaptive verify strength, Spec 3/H) — the LAST v11.0 phase. After it: v11.0 milestone-close (separate gate).
...
- **v11.0 State Machine Completion — in progress (1/3 phases; Phase 34 done).** Opened 2026-06-25 to close the v5.0 deferred specs. Full scope D+G+H (user-chosen). Sketch-then-refine: ROADMAP carries phase one-liners; full PLAN per-phase at plan time.
  ...
  - **Phase 36 — PLANNED (ready-to-execute).** scale-adaptive verify strength (Spec 3/H) — LAST v11.0 phase. Research: `assessScale` + `checkpoint complete` already compute+store `verify_mode` on the envelope but NOTHING renders it. Design: surface a `VERIFY-MODE: <mode> — <directive>` advisory line in the breadcrumb (`buildWorkflowStateBlock` + bin parity), mirroring SHIP-READY/RETRO-DUE. Plan: `phases/36-adaptive-verify/`. 2 files + tests, no schema change.
...
- **v11.0 OPENED; Phase 34 + 35 DONE; Phase 36 PLANNED 2026-06-25** (v10.0 shipped+published npm 4.8.0 prior). Strategy gate skipped (scope in v5.0 design, declared). Full scope D+G+H. **Next: execute Phase 36** — TDD: T36.1 surface `verify_mode` in `buildWorkflowStateBlock` + bin parity (mirror SHIP-READY/RETRO-DUE) → T36.2 gate+e2e. Plan at `phases/36-adaptive-verify/`. Baseline 1466 tests. After Phase 36: v11.0 COMPLETE → milestone-close (LIGHTWEIGHT, separate gate). Per 逐-gate, await user word before each commit/phase-advance.
