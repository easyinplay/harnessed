...
- **GSD phase**：✅ **Phase 3.1 SHIPPED**（2026-05-16 — checkpoint 引擎 TEMPLATE 摘要 (zero LLM call) + archive 双轨 unbounded raw turn dump + `harnessed resume` 12th CLI subcommand (D-03 RELOAD + cwd guard) + compact 75% threshold MVP placeholder + T4.4 closure infra 三件套 1 milestone 闲置后**首消费者** activation 闭环 (D-04 WIRE-IN 实证 — Phase 2.2 ship `sdkSpawn.onSessionId` + `ralphLoopWrap.resumeSessionId` + `engine.wrappedSpawn capturedSessionId`) + W-01 engineHook.ts PRIMARY extract engine.ts ≤200L Karpathy clean restored + W-04 phaseId via task schema 删 `as any` cast + Wave 0 backlog 4 项一次根治；F1-F8 8/8 acceptance bar；ADR 0014 9 章节 errata accepted；14 ADR + 14 baseline tag；v0.3.0 MILESTONE 1/4 START）— 前置 ✅ **Phase 2.4 SHIPPED**（2026-05-16）+ 🎯 **v0.2.0 MILESTONE 4/4 SHIPPED & ARCHIVED**（2026-05-16）；前置 ✅ **Phase 2.3 SHIPPED** 2026-05-16；前置 ✅ **Phase 2.2 SHIPPED** 2026-05-15；前置 ✅ Phase 2.1 SHIPPED；前置 ✅ v0.1.0 SHIPPED & ARCHIVED（6/6 phase；git tag `v0.1.0`）
- **当前里程碑**：v0.3.0 plan-feature workflow + checkpoint（**Phase 3.1 SHIPPED 1/4**）— next: Phase 3.2 gstack 前缀探测 + workflow 变量插值 + plan-feature reference 实装
- **下一 phase**：**Phase 3.2 discuss-phase 启动** — gstack 前缀探测 (R02 § 1 doctor 三选一) + workflow 变量插值 (`invokes` 字段插值) + plan-feature reference 实装 + governance 层 pause + on_veto + DEFERRED #1 W0.3 ENFORCE flip prereq (STATE/README format normalization)；候选启动 `/gsd-discuss-phase 3.2`
- **状态**：✅ **Phase 3.1 SHIPPED** — Wave 0+1+2+3+4+5 全 ship 2026-05-16；A7 守恒 ADR 0001-0013 main body 0 diff（T5.4 ship-time verify PASS）；CI 3-OS green；tests 543→596 (+53) + 4 skipped；ADR 0014 9 章节 errata accepted；T4.4 closure infra 三件套 RESOLVED (Phase 2.2 ready → Phase 3.1 W3 activated)；engine.ts 195→200L Karpathy clean ≤200L restored (W-01 PRIMARY extract)
- **进度**：11 / 17 phases 已完成 ▓▓▓▓▓▓▓▓▓▓▓░░░░░░ 64.7%（v0.1.0 里程碑 100%；v0.2.0 里程碑 100%；v0.3.0 里程碑 25% — Phase 3.1 ✅）
