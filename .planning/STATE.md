...
- **Phase 54 Declaration/Evaluation Parity(2026-08-26 立项,ready-to-execute)**:三道关卡全过(office-hours 推翻原「完整 codex 支持」指示 → CEO HOLD_SCOPE 6 findings + outside voice → ENG 3 findings)。规划期新量出病族第三面且**有行为后果**:`phases[].max_iterations` 21 处模板引用全部失效(`loadPhases` 只插值 `invokes`),每个 phase 实跑默认 20 而非声明值,5 处连 defaults 键都对不上。T0-T7 / 4 波次详 `phases/54-declaration-evaluation-parity/SPEC.md`;决策链详设计文档(gstack projects)。
