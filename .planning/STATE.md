...
> Status: v0.1.0 SHIPPED & ARCHIVED · **🎯 v0.2.0 MILESTONE 4/4 SHIPPED & ARCHIVED** (2026-05-16) · **Phase 3.2 SHIPPED** (2026-05-17 — 4 Waves W0-W3 全 ship 24+ atomic commits; gstack 前缀探测 PROBE + workflow JINJA 插值 + plan-feature 5-phase WIRED reference + governance.json PUSH; ADR 0015 9 章节 errata accepted; v0.3.0 milestone 第 2 phase ship) · **Phase 3.1 SHIPPED** (2026-05-16 — 6 Waves W0-W5 全 ship 27+ atomic commits; checkpoint 引擎 TEMPLATE + archive 双轨 + harnessed resume 12th CLI + compact 75% placeholder + T4.4 closure infra activation 闭环 (D-04 WIRE-IN 实证); ADR 0014 9 章节 errata accepted; v0.3.0 milestone 第 1 phase ship) · **Phase 2.4 SHIPPED** (2026-05-16 — 7 Waves W0-W6 全 ship 25+ atomic commits; doctor MIN 5 check + EE-4 plan-checker 4 维 SSOT + dashboard C 路径 3 子 FULL absorb + audit hard-fail + ralph-loop Win sentinel 5 fixture; ADR 0013 9 章节 errata accepted; v0.2.0 milestone close)
> 最后更新：2026-05-17（**Phase 3.2 SHIPPED** 2026-05-17 — gstack 前缀探测 PROBE + workflow JINJA `{{prefix}}` 插值 + plan-feature 5-phase WIRED reference + governance.json PUSH veto halt_workflow；ADR 0015 9 章节 errata accepted；15 ADR + 15 baseline tag；T4.4 closure infra 二代消费者闭环；v0.3.0 milestone 2/4 PROGRESS。前置 **Phase 3.1 SHIPPED** 2026-05-16；**🎯 v0.2.0 MILESTONE 4/4 SHIPPED & ARCHIVED** 2026-05-16 — 4 phase ship (Phase 2.1+2.2+2.3+2.4) + 13 ADR baseline tag + 9 milestone tag accumulate + git tag `v0.2.0` pushed；archive `.planning/milestones/v0.2.0-{ROADMAP,REQUIREMENTS}.md` + audit `.planning/v0.2.0-MILESTONE-AUDIT.md` ship。**Phase 2.4 SHIPPED** 2026-05-16 — 7 Waves W0-W6 全 ship 25+ atomic commits across W0-W5 main + W6 final ship + deferred-items #3 RESOLVED (EE-4 step reorder fix)；详 L24-25 当前位置块。 前置：**Phase 2.3 SHIPPED** 2026-05-16；前置 **Phase 2.2 SHIPPED** 2026-05-15；前置 **Phase 2.1 SHIPPED**；前置 **🎯 v0.1.0 MILESTONE SHIPPED & ARCHIVED**：6/6 phase + audit passed + git tag v0.1.0）
...
- **GSD phase**：✅ **Phase 3.2 SHIPPED**（2026-05-17 — gstack 前缀探测 PROBE doctor 6th check (sister Phase 2.4 doctor MIN 5-check + `--json` CI-friendly) + workflow `invokes` JINJA `{{prefix}}` 模板替换 (zero npm dep + cross-OS + Karpathy YAGNI strict) + plan-feature 5-phase WIRED reference (workflow.run.ts ≤80L + 5 SKILL.md stubs mock + plan-feature/workflow.yaml DSL + activatePhase BEFORE isVetoed B-01 fix locked) + governance.json PUSH veto halt_workflow (gstack 写 harnessed 读 file-based zero coordination + sister Phase 3.1 file-based state machine 模式) + Wave 0 backlog 3 项一次根治；F1-F8 8/8 acceptance bar；ADR 0015 9 章节 errata accepted；15 ADR + 15 baseline tag；T4.4 closure infra 二代消费者闭环 (Phase 3.1 W3 engineHook 首消费者 + Phase 3.2 W2 governance.json PUSH 二代)；v0.3.0 MILESTONE 2/4 PROGRESS）— 前置 ✅ **Phase 3.1 SHIPPED**（2026-05-16 — checkpoint 引擎 TEMPLATE + archive 双轨 + harnessed resume 12th CLI + compact 75% placeholder + T4.4 closure infra activation 闭环 D-04 WIRE-IN 实证 + ADR 0014 9 章节）；前置 ✅ **Phase 2.4 SHIPPED**（2026-05-16）+ 🎯 **v0.2.0 MILESTONE 4/4 SHIPPED & ARCHIVED**（2026-05-16）；前置 ✅ **Phase 2.3 SHIPPED** 2026-05-16；前置 ✅ **Phase 2.2 SHIPPED** 2026-05-15；前置 ✅ Phase 2.1 SHIPPED；前置 ✅ v0.1.0 SHIPPED & ARCHIVED（6/6 phase；git tag `v0.1.0`）
- **当前里程碑**：v0.3.0 plan-feature workflow + checkpoint（**Phase 3.1 + Phase 3.2 SHIPPED 2/4**）— next: Phase 3.3 aliases.yaml + deprecation marker + known-good 版本组合
- **下一 phase**：**Phase 3.3 plan-phase 启动** (discuss-phase 已 ship 2026-05-17) — 4 D-decision 全 Recommended lock: D-01 RICH aliases (含 reason+since_version+deprecation_date) + D-02 DOCTOR-ONLY-WARN deprecation visibility + D-03 YAML manifest `versions/<ver>-known-good.yaml` + D-04 (b) COLLAPSE STATE dual-SSOT 5-recurrence terminus；W0 backlog 3 项 absorb: W0.1 (D-04 实施 STATE collapse + freshness gate STATE_POSITION_RE 扩) + W0.2 (Phase 3.2 deferred #2 dashboard-sse 4 cell flaky 根治 random port path) + W0.3 (schemaVersion 12+13 surface aliases.v1 + known-good.v1 decision doc)；候选启动 `/gsd-plan-phase 3.3`
- **状态**：✅ **Phase 3.2 SHIPPED** — Wave 0+1+2+3 全 ship 2026-05-17；A7 守恒 ADR 0001-0014 main body 0 diff（T3.4 ship-time verify PASS）；CI 3-OS green；tests 611→623+ (+12) + 4 skipped；ADR 0015 9 章节 errata accepted；schemaVersion 8→11 surface (config.v1 + governance.v1 + planFeature.v1)；前置 Phase 3.1 W3 T4.4 closure infra activation 闭环 + Phase 3.2 W2 governance.json PUSH 二代消费者 闭环
- **进度**：12 / 17 phases 已完成 ▓▓▓▓▓▓▓▓▓▓▓▓░░░░░ 70.6%（v0.1.0 里程碑 100%；v0.2.0 里程碑 100%；v0.3.0 里程碑 50% — Phase 3.1 + 3.2 ✅）
- **Phase 2.4 关键决议 ship 总结**：(1) doctor MIN 5 check (D-01) — `src/cli/doctor.ts` 4 → 5 check (+origin URL warn) + `--json` flag + 三档 status enum (pass/warn/fail) + helper `origin-check.ts` sister-share；(2) EE-4 plan-checker 4 维 SSOT (D-02) — `routing/plan-review-schema.yaml` 4 维 + `scripts/run-plan-checker.mjs` walker + T2.0 RELAX baseline (3 阈值 1.0 → 0.8) + BLOCKER manual rerun (auto-spawn 推 v0.3.0)；(3) dashboard C 路径 3 子功能 FULL absorb (D-04) — cc-hook-add 第 7 install method + SSE watcher (替 mtime polling, zero-dep) + 多项目支持 + 3 处 schema enum 同步加；(4) audit 完整版 hard-fail (R2.4.7) — origin URL drift hard-fail (allowFork: false) + cmd injection SHELL_EVAL_MARKERS refinement + provenance cross-check；(5) ralph-loop Win sentinel (F5) — 5 fixture (simple/multi-iter/max-iter/subagent-mock/structured_output) + `if: runner.os == 'Windows'` + `shell: bash`；(6) README CI counter gate (D-03 B 路径) — 三计数一致 grep -cE 精度匹配 (SHIPPED == BARS == L44) + line-start + bold + Phase 2.[1-9] 精度排除 1.X 历史；(7) **Wave 0 backlog 5 项一次根治** — H3+T4 README counter / M1 RETRO dashboard polish round 1 / M2 schemaVersion 3rd consumer (check-provenance.mjs) / T2 dashboard absorb 决断 / T3 v0.3.0 prereq 注记；(8) deferred-items #3 RESOLVED — EE-4 step reorder fix (move AFTER pnpm install) 1-line ci.yml 修；(9) **v0.2.0 MILESTONE 4/4 CLOSED** — 4 phase ship (Phase 2.1+2.2+2.3+2.4) + 13 ADR + 13 baseline tag + 9 milestone tag accumulate + archive + audit ship
...
| v0.2.0 Sub-task Loop + Extension Installers | 4/4 | 🎯 **SHIPPED & ARCHIVED** — Phase 2.1 + Phase 2.2 SHIPPED 2026-05-15; **Phase 2.3 SHIPPED 2026-05-16**; **Phase 2.4 SHIPPED 2026-05-16** (doctor MIN 5 + EE-4 SSOT + dashboard C 路径 + audit hard-fail + Win sentinel + Wave 0 backlog 5 项); ADR 0013 9 章节 errata; 13 ADR + 13 baseline tag accumulate; archive + audit ship | 启动 2026-05-15 / 全 4 phase ship 2026-05-15~16 / milestone close 2026-05-16 |
| v0.3.0 plan-feature + checkpoint | 2/4 | 🚀 **PROGRESS — Phase 3.1 SHIPPED 2026-05-16 + Phase 3.2 SHIPPED 2026-05-17** (P3.1: checkpoint 引擎 + harnessed resume + compact + T4.4 closure activation 闭环 + ADR 0014; P3.2: gstack PROBE + JINJA 插值 + plan-feature 5-phase WIRED + governance.json PUSH + ADR 0015 + T4.4 closure infra 二代消费者闭环); tests 543→623+ (+80); 15 ADR + 15 baseline tag; schemaVersion 7→11 surface; next: Phase 3.3 aliases.yaml + deprecation marker + known-good 版本组合 + DEFERRED #2 dashboard-sse 一次根治 + H3 STATE dual-SSOT 根治 | 启动 2026-05-16 / Phase 3.1 ship 2026-05-16 / Phase 3.2 ship 2026-05-17 |
...
22. ✅ ~~**Phase 2.3 SHIPPED**~~ (2026-05-16) — extension category MVP + karpathy SKILL-ONLY 注入 + EE-5 5-question CLI gate + CD-3 negative-space arbitrate redirect + 30/30 routing 100% (≥85% bar 远超 15% headroom) + Wave 0 backlog 6 项一次根治；ADR 0012 9 章节 errata accepted；12 ADR + 12 baseline tag iterate (`adr-0012-accepted` + `v0.2.0-alpha.3-extension-mvp` milestone tag Wave 6 T6.4 push)；A7 守恒 ADR 0001-0011 main body 0 diff (Wave 5 T5.4 mid-wave + Wave 6 T6.4 ship-time 双 verify)；DI-1 hotfix karpathy-skills.yaml schema-compliant；3 sister review backlog T-W6-x 全 absorb；CI 3-OS green；tests 432→492 (+60)；详 `.planning/phase-2.3/{progress.md, VERIFICATION.md}` + `docs/adr/0012-extension-mvp-karpathy-inject.md`
...
23a. ⏳ **v0.3.0 W0 prereq backlog (Phase 2.4 sister M+L+T defer)**: M1 audit "Line budget deviations accepted" 透明节加 (deferred-items #1 #2 — run-plan-checker.mjs 130L+30% / plan-checker-quant.test.ts 103L+29% biome blocking ACCEPTED) / M2+M3 dashboard polish round 2 commit attribution 复盘 (polish 2 changes 实际 absorb 进 W3 cf00d17/008f9ab — handoff intel L482 草案"独立 commit"未采纳，advisory 允许 absorb 路径) / T1+T3 不再 defer (H1+H2 已 800e459 inline 修) / T4 README completeness check (STATE shipped count vs README shipped count — counter gate 现 consistency-only) / T5 phase-2.4/task_plan.md self-check fr ≥ 0.80 end-state verify (`node scripts/run-plan-checker.mjs .planning/phase-2.4/` real-run)
24. ✅ ~~**Phase 2.4 backlog 决断**~~：EE-4 plan 4 维量化阈值 schema (omo ⭐⭐) — Phase 2.4 doctor 完整版 D-02 absorb ship (W2 T2.0-T2.4)
25. ✅ ~~**Phase 2.4 Wave 0 SHIPPED**~~ (2026-05-16) — 6 atomic commits T0.1-T0.6 + 1 pre-flight tooling commit (3fc0c42 dashboard polish round 2)；STATE.md L601+ Phase 2.4 Prereq Notes 5 项 backlog 一次根治；ADR 0013 9 章节 sketch draft；CI run 25954285185 3-OS 全绿 ✅
26. ✅ ~~**Phase 2.4 Wave 1-5 SHIPPED**~~ (2026-05-16) — 24 atomic commits W1 doctor (T1.1-T1.3) + W2 EE-4 (T2.0-T2.4) + W3 dashboard C 路径 (T3.1-T3.4 incl. T3.4 SKIP split decision) + W4 audit + Win sentinel (T4.1-T4.3 + T4.1-fix Rule 1) + W5 30-fixture matrix (T5.1-T5.3); tests 432→543 (+111) / 4 skipped; CI run 25958484645 head `1b108be` 3-OS 全绿 ✅
27. ✅ ~~**Phase 2.4 SHIPPED**~~ (2026-05-16) — Wave 6 final ship (T6.1 ADR 0013 finalize 185L 9 章节 + T6.2 ci.yml A7 iter 1-0013 + deferred-items #3 EE-4 step reorder fix + T6.3 STATE/RETRO 续编 + T6.4 baseline tag + v0.2.0 milestone tag + T6.5 milestone archive)；F1-F8 8/8 acceptance bar；ADR 0013 9 章节 errata accepted；A7 守恒 ADR 0001-0012 main body 0 diff (T6.2 ship-time verify 0 lines)
28. 🎯 ~~**v0.2.0 MILESTONE 4/4 SHIPPED & ARCHIVED**~~ (2026-05-16) — 4 phase (Phase 2.1+2.2+2.3+2.4) ship + 13 ADR + 13 baseline tag + 9 milestone tag accumulate；archive `.planning/milestones/v0.2.0-{ROADMAP,REQUIREMENTS}.md`；audit `.planning/v0.2.0-MILESTONE-AUDIT.md`；git tag `v0.2.0` pushed；next: v0.3.0 discuss-phase 启动
29. ✅ ~~**Phase 2.4 ship sister H1+H2 atomic absorb**~~ (2026-05-16, commits `800e459` + `b6a0feb`) — README 6 stale 处一次根治 (L9 Status / L41 heading / L44 milestone / L45 NEW Phase 2.4 entry / L57 NEW acceptance bar / L58 Next; 4th recurrence terminus) + freshness gate 2 sub-bug root-cause 修 (Bug 1: getLatestShippedToken `.match()` 没 `/g` 返回 v0.1.0 oldest not latest → matchAll + last; Bug 2: STATUS_MARKER `.includes()` substring 太弱 "Next: Phase X.Y" 命中误 PASS → milestone 需 SHIPPED/MILESTONE/ARCHIVED 80字符上下文 regex + subphase 需 literal `**Phase X.Y SHIPPED`); pre-fix gate "all compliant" silent PASS, post-fix verify gate 正确 FAIL "missing subphase literal **Phase 2.4 SHIPPED" 捕到; build pass + 543 tests / 4 skipped 与 ship baseline 一致 0 regression; T0.4 freshness gate 第二次结构性硬化 (Phase 2.2 W0 round 1 = 加 STATUS_MARKER; ship 后 round 2 = 修 2 sub-bug); E11 update: freshness gate 实际 catch 二次 (PROJECT-SPEC 61ba0d4 + README 800e459); **counter gate post-close 格式 hotfix `b6a0feb`** — L44 加 "完成" literal + ci.yml regex `(/2\.4)?` 兼容 in-progress + closed 两种 format (CI ddd5169 3-OS fail → b6a0feb 3-OS green 闭环)
30. 🚀 ~~**Phase 3.1 discuss-phase 启动**~~ (2026-05-16) — v0.3.0 milestone 第 1 phase, checkpoint 引擎 + `harnessed resume` + compact 协议. 4 D-decision 一次 batch 决策全 Recommended option lock: D-01 TEMPLATE 摘要 (zero LLM call, R7.2 验收 < 1k token 指向机械拼装) / D-02 KARPATHY 极简 3 state (active/paused/complete, YAGNI 不开 6-state FSM) / D-03 RELOAD resume (不偷袭用户, 与 Claude Code 交互底色符) / D-04 WIRE-IN T4.4 closure infra (Phase 2.2 ready 三件套首消费者, lossless session 续跑). W0 backlog 4 项 absorb (M1 audit 透明节 + M2+M3 dashboard polish RETRO 复盘 + T4 README completeness check + T5 phase-2.4 self-check). 详 `.planning/phase-3.1/{3.1-KICKOFF, 3.1-CONTEXT, 3.1-DISCUSSION-LOG}.md`. 下一步: `/gsd-plan-phase 3.1`
31. 📋 ~~**Phase 3.1 plan-phase READY TO EXECUTE**~~ (2026-05-16) — Wave A 并行 (R1 gsd-pattern-mapper PATTERNS.md 363→412L 15 files mapped + R2 gsd-phase-researcher RESEARCH.md 990L 12 sections) → Wave B gsd-planner (PLAN.md 1238L + task_plan.md 1228L, 27-28 atomic tasks × 6 waves W0-W5) → Wave C gsd-plan-checker iter 1 (2 BLOCKER + 4 WARNING; EE-4 4/4 PASS RELAX baseline) → revision iter 1 (6/6 fix per orchestrator tier-call decisions: B-01 W0.3 warn-only round 1 sister Phase 2.1 transparency pattern + B-02 userSpawn fresh-session-only documented + W-01 engineHook.ts PRIMARY extract engine.ts ≤200L Karpathy clean restored + W-02 explicit `ADR 0001-0014` literal + W-03 vitest config glob verify + W-04 phaseId via task schema 删 `as any`) → Wave C iter 2 (**VERIFICATION PASSED**, EE-4 4/4 持稳, D-01~D-04 全 hold strengthened) → I-01 inline polish (PATTERNS.md +row 20 engineHook.ts analog sdkReconcile.ts ≤56L Phase 2.2). 详 `.planning/phase-3.1/{PATTERNS, RESEARCH, PLAN, task_plan, PLAN-CHECK, PLAN-CHECK-DELTA}.md`. 6 wave topology: W0 backlog 4 项 absorb + setup / W1 schema + state 3-state / W2 template + archive 双轨 / W3 SDK wire + ralphLoop activation + engineHook PRIMARY / W4 SIGINT trap + CLI 12th + compact placeholder 75% / W5 e2e dogfood + ADR 0014 9 章节 + ship tags. 下一步: `/gsd-execute-phase 3.1`
32. 🔧 ~~**Phase 3.1 Wave 0 SHIPPED**~~ (2026-05-16) — 5 sub-tasks 全 absorb (4 commits + 1 inline T0.4 verify): b0ab4f7 T0.5 tests/checkpoint/cli/integration dirs scaffold + vitest glob verify / 748af7e T0.1 AUDIT.md § 0.5 Line Budget Deviations Accepted (doctor.ts 215L +7% B-03 5% tolerance + dashboard.mjs 610L well-under ≤650L Phase 2.4 D-04 absorb 透明登记) / ee6e2cd T0.2 RETROSPECTIVE.md Advisory Absorb Path 节 (dashboard polish round 2 cf00d17/008f9ab absorb cadence note avoid "advisory rejected" 暗示) / T0.3 ci.yml README completeness check WARN-ONLY round 1 + T0.4 phase-2.4 self-check PASS (PLAN+task_plan 3/4 WARNING fr ≥0.80 → deferred-items #2 self-referential 解决证据, no auto_retrigger). **DEFERRED #1 (Phase 3.1 W0.3): README completeness check ENFORCE flip → Phase 3.2 W0 prereq, after STATE.md/README.md format normalization (sister Phase 2.3 EE-4 BLOCKER auto-spawn defer pattern)**. 下一步: Wave 1 schema 8th surface + state.ts 3-state machine 启动
33. ✅ ~~**Phase 3.1 Wave 1-4 SHIPPED**~~ (2026-05-16) — 18 atomic commits across W1+W2+W3+W4: W1 schema 8th surface + state.ts 3-state machine 4 task (8441bda T1.1 schemaVersion 8th surface currentWorkflow + TaskContext.phaseId field / b2bc907 T1.2 checkpoint + currentWorkflow TypeBox schemas NEW 88L / acd6724 T1.3 checkpoint/state.ts 76L D-02 KARPATHY 3-state machine / 7254f75 T1.4 checkpoint schema/state/engineHook tests + 2 Rule-1 fixes 13 active + 3 todo); W2 template + archive 双轨 4 task (4bff906 T2.1 checkpoint/template.ts D-01 zero-LLM writer + budget enforce / 571d32f T2.2 checkpoint/archive.ts 双轨 unbounded raw turn dump / 2119d80 T2.3 checkpoint/index.ts barrel 4-module re-export / 2d49b26 T2.4 checkpoint template + archive tests 11 fixtures + D-01 grep gate); W3 SDK wire-in + engineHook PRIMARY extract (D-04 WIRE-IN) 3 task (6c0d3a9 T3.1 ralphLoop.ts sessionId capture activation D-04 WIRE-IN / 5c1f024 T3.2 engineHook.ts PRIMARY + engine.ts wedge D-04 WIRE-IN + W-01 + W-04 + B-02 / 791848e T3.3 sdk-wire.test.ts 7 fixture + W1 engineHook stub absorb); W4 SIGINT trap + CLI resume + compact placeholder 5 task (4350e03 T4.1 SIGINT trap Node native + isShuttingDown + 30s timeout / 0851227 T4.2 compact threshold placeholder shouldCompact 75% / 932d423 T4.3 resume.ts D-03 RELOAD + cwd guard + branchOnSchemaVersion / 2a279c6 T4.4 12th CLI subcommand harnessed resume + barrel export / 4e9c4ef T4.5 sigint/compact/resume tests 18 fixtures all pass); engine.ts 195→200L Karpathy clean ≤200L restored (W-01 PRIMARY extract engineHook.ts 49L); 0 `as any` cast (W-04 phaseId via TaskContext typed field); 4 D-decisions全 hold strengthened; tests 543 → 590 (+47). 下一步: Wave 5 e2e dogfood + ADR 0014 + ship + v0.3.0 alpha.1 close
34. ✅ ~~**Phase 3.1 SHIPPED**~~ (2026-05-16) — Wave 5 ship close (T5.1 e2e SIGINT → resume → SDK redirect 3 fixtures pass `537b10a` + T5.2 ADR 0014 9 章节 errata 173L ≤250 Karpathy `524eeb2` + T5.3 STATE/RETRO/ROADMAP 续编 + T5.4 ci.yml A7 iter `1-0013` → `1-0014` per W-02 explicit literal + T5.5 Phase 3.1 dogfood DOGFOOD-T5.5.md + T5.6 baseline tag `adr-0014-accepted` + milestone tag `v0.3.0-alpha.1-checkpoint`)；F1-F8 8/8 acceptance bar；ADR 0014 9 章节 errata accepted；A7 守恒 ADR 0001-0013 main body 0 diff (T5.4 ship-time verify 0 lines)；T4.4 closure infra 三件套 1 milestone 闲置后**首消费者** activation 闭环 (D-04 WIRE-IN 实证 — Phase 2.2 ready → Phase 3.1 W3 activated)；engine.ts 195→200L Karpathy clean ≤200L restored (W-01 PRIMARY extract engineHook.ts 49L sister sdkReconcile.ts 56L)；0 `as any` cast (W-04 phaseId via TaskContext typed field)；tests 543 → 596 (+53)；27+ atomic commits across W0-W5；v0.3.0 MILESTONE 1/4 START
35. ⏳ **v0.3.0 W0 prereq backlog (Phase 3.1 → Phase 3.2 carry-forward)**: DEFERRED #1 (Phase 3.1 W0.3) README completeness check ENFORCE flip → Phase 3.2 W0 prereq after STATE.md/README.md format normalization (sister Phase 2.1 transparency 4-phase warn-only round 1 → ENFORCE pattern); **cli-audit env-dep CI red → Phase 3.2 W0 FIRST TASK 必修** (Phase 3.1 deferred #1 priority bump per post-ship 0d16496; Phase 2.4 W4 audit runtime-layer 后 unit test mocks 未同步, root cause + fix path locked in `.planning/phase-3.1/deferred-items.md`); DEFERRED #2 userSpawn session_id capture niche → Phase 3.4+ if real demand (fresh-session fallback currently acceptable per B-02)
36. 🚀 ~~**Phase 3.2 discuss-phase 启动**~~ (2026-05-16) — v0.3.0 milestone 第 2 phase, gstack 前缀探测 + workflow 变量插值 + plan-feature reference 实装. 4 D-decision 一次 batch 决策全 Recommended option lock: D-01 PROBE doctor 运行时探三选一 + fallback (sister Phase 2.4 doctor `--json` CI-friendly 经验) / D-02 JINJA `{{ prefix }}` 模板替换 (zero-dep + cross-OS + Karpathy YAGNI) / D-03 WIRED 5 phase 桩 + governance gate 接不接外 (中庸, Phase 3.3+ dogfood 换真) / D-04 PUSH gstack veto 写 .harnessed/governance.json → workflow next-phase 读 (zero coordination + 沿袭 Phase 3.1 current-workflow.json file-based state machine). W0 backlog 3 项 absorb (W0.1 必修 cli-audit env-dep CI red fix + W0.2 README completeness ENFORCE flip + W0.3 doctor PROBE schemaVersion 9th surface 决策). 详 `.planning/phase-3.2/{3.2-KICKOFF, 3.2-CONTEXT, 3.2-DISCUSSION-LOG}.md`. 下一步: `/gsd-plan-phase 3.2`

---

## 已完成（Completed）

- 🎯 **v0.2.0 MILESTONE 4/4 SHIPPED & ARCHIVED**（2026-05-16）
  - **4 phase ship**: Phase 2.1 (installers 6 method runtime) + Phase 2.2 (execute-task SDK + ralph-loop full) + Phase 2.3 (extension MVP + karpathy SKILL-ONLY + 30/30 routing) + Phase 2.4 (doctor 完整版 + EE-4 SSOT + dashboard C 路径 + audit hard-fail + Win sentinel)
  - **13 ADR + 13 baseline tag accumulate**: 0001-0013 (含 0010-installer-schema-errata / 0011-execute-task-sdk-ralph / 0012-extension-mvp-karpathy-inject / 0013-doctor-ee4-dashboard-audit-win)
  - **9 milestone tag accumulate**: alpha.1-schema-frozen / alpha.2-installer-runtime / alpha.3-base-profile / alpha.4-routing-engine / alpha.5-routing-l2-engineering / v0.2.0-alpha.1-installers / v0.2.0-alpha.2-execute-task / v0.2.0-alpha.3-extension-mvp / **v0.2.0-alpha.4-doctor** + 🎯 **v0.2.0** (大里程碑 tag)
  - **Tests baseline cumulative**: 318+3 (v0.1.0 end) → 374+3 (Phase 2.1) → 432+3 (Phase 2.2) → 492+3 (Phase 2.3) → **543+4 (Phase 2.4)** (+225 cells across v0.2.0)
  - **A7 守恒**: ADR 0001-0013 main body 0 diff 持续 verified ci.yml 自动化 step (4 phase × Wave 6 ship-time double-verify)
  - **Archive**: `.planning/milestones/v0.2.0-ROADMAP.md` + `.planning/milestones/v0.2.0-REQUIREMENTS.md` (Wave 6 T6.5) + audit `.planning/v0.2.0-MILESTONE-AUDIT.md` (sister Phase 1.5 v0.1.0 close 模板)
  - 见 `.planning/v0.2.0-MILESTONE-AUDIT.md` + `.planning/milestones/v0.2.0-*.md`
- ✅ **Phase 2.4 SHIPPED**（2026-05-16）
  - **25+ atomic 子任务全 ship across 7 Waves W0-W6** (W0 6 task T0.1-T0.6 + W1 3 doctor + W2 5 EE-4 + W3 4 dashboard C 路径 incl. T3.4 SKIP + W4 3 audit/Win sentinel + W5 3 30-fixture matrix + W6 5 ship — 含 deferred-items #3 EE-4 yaml fix RESOLVED；详 `.planning/phase-2.4/{progress.md, VERIFICATION.md}` + `docs/adr/0013-phase-2.4-doctor-ee4-dashboard-c-path.md`)
  - **Acceptance bar F1-F8 8/8** ✅
    - F1 ADR 0013 errata accepted — `docs/adr/0013-phase-2.4-doctor-ee4-dashboard-c-path.md` Wave 0 draft (T0.2 9 章节 skeleton sketch 126L) → Wave 6 fill-out (T6.1 detailed expand 185L) → Status `Accepted (phase 2.4 W6 — 2026-05-16)`；9 章节 ≤ 250L Karpathy hard limit
    - F2 doctor MIN 5 check ship (R2.4.1) — Wave 1 T1.1-T1.3: `src/cli/doctor.ts` 152→~215L (5% 容忍超 hard limit per B-06) + `src/cli/lib/origin-check.ts` ~52L sister-share helper + 三档 status enum (pass/warn/fail) + `--json` flag CI 友好输出；30-fixture matrix (5 check × 6 env scenario) Wave 5 T5.1 verify pass
    - F3 EE-4 plan-checker 4 维 SSOT (R2.4.2) — Wave 2 T2.0-T2.4: `routing/plan-review-schema.yaml` NEW yaml-only SSOT 4 维 (file_references_verified / reference_sources_real / concrete_acceptance / business_logic_assumptions) + T2.0 RELAX baseline (3 阈值 1.0 → 0.8) + `scripts/run-plan-checker.mjs` walker (130L per deferred-items #1 tolerance) + BLOCKER manual rerun (auto-spawn 推 v0.3.0 per B-12)
    - F4 dashboard C 路径 3 子功能 FULL absorb (R2.4.4-R2.4.6) — Wave 3 T3.1-T3.4: cc-hook-add 第 7 install method NEW (sister Phase 2.1 6 → 7 dispatch swap) + SSE watcher 替 mtime polling (zero-dep + `text/event-stream` HTTP + browser-native EventSource reconnect) + 多项目支持 (`/api/projects` + project selector + URL `?project=<path>` + auto-init cwd) + 3 处 schema enum 同步加 (InstallType +'hook' / TypeEnum +'cc-hook' / install.method +'cc-hook-add')；T3.4 SKIP split decision dashboard.mjs 610L ≤ 650L single-file maintained per B-26 default
    - F5 audit 完整版扩 hard-fail + Win sentinel (R2.4.7) — Wave 4 T4.1-T4.3: `src/cli/audit.ts` 扩 origin URL drift hard-fail (sister-share origin-check.ts allowFork:false 模式) + install.cmd shell-eval injection (SHELL_EVAL_MARKERS `/$$\\(|$$\\{|`/` T4.1-fix Rule 1 refinement security.ts B1 alignment) + provenance cross-check；Win sentinel 5 fixture (simple/multi-iter/max-iter/subagent-mock/structured_output) + `if: runner.os == 'Windows'` + `shell: bash` (per B-04)
    - F6 30-sample integration e2e (R2.4.7 ext / D2.4-19 anti-redo) — Wave 5 T5.1-T5.3: doctor 30-fixture matrix (5 check × 6 env scenario) + plan-checker 30 fixture (8 real phase-1.1~2.3 PLAN+task_plan + 14 synthetic spectrum + 8 cells T5.1 = 30) + phase-2.4 e2e 6-link cross-compose (doctor / EE-4 walker+schema / dashboard SSE+multi-proj+localhost-bind / cc-hook-add 7th installer / audit-helpers + Win sentinel / cross-link compose)
    - F7 Wave 0 backlog 5 项一次根治 (W0 T0.3-T0.6) — H3+T4 README counter integrity CI gate B 路径 (root-cause-class 第 3 次复发根治) / M1 dashboard polish round 1 RETRO entry / M2 schemaVersion 3rd consumer (`scripts/check-provenance.mjs` land) / T2 dashboard 进 doctor 完整版 absorb 决断 (D-04 FULL Wave 3 ship) / T3 v0.3.0 启动 prereq 注记 (STATE.md L628+)
    - F8 SHIP — `adr-0013-accepted` baseline tag + `v0.2.0-alpha.4-doctor` milestone tag + 🎯 **`v0.2.0` 大里程碑 tag** (Wave 6 T6.4 main agent push)；ci.yml A7 step iter `1-0012` → `1-0013` (T6.2)；A7 守恒 ADR 0001-0012 main body 0 diff verified (T6.2 ship-time verify `git diff adr-0012-accepted..HEAD -- "docs/adr/000[1-9]-*.md" "docs/adr/001[0-2]-*.md" | wc -l = 0`)；CI 3-OS final verify (T6.4 push 触发)；EE-4 step ordering bug fix deferred-items #3 RESOLVED (1-line step reorder AFTER pnpm install)
  - **7 wave 完整跑完**：Wave 0 (6 task — backlog 5 项根治 + ADR 0013 sketch + tests dirs + check-deferred-items.mjs cadence) / Wave 1 (3 task — doctor 5 check + origin-check helper + doctor.test) / Wave 2 (5 task — T2.0 spike + plan-review-schema.yaml + walker + quant test + ENFORCE warn-only) / Wave 3 (4 task — cc-hook-add + SSE watcher + 多项目 + T3.4 SKIP split decision) / Wave 4 (3 task — audit hard-fail + audit unit + Win sentinel 5 fixture) / Wave 5 (3 task — doctor 30 fixture + plan-checker 30 fixture + e2e 6-link) / Wave 6 (5 task — ADR 0013 9 章节 finalize + ci.yml A7 iter + EE-4 step reorder + STATE+RETRO 续编 + baseline tag + milestone archive)
  - **discuss-phase 5 大主题 absorbed (5 sister cluster 一次 ship)**：D-01 doctor MIN 5 check + D-02 EE-4 plan-checker 量化 SSOT + D-03 README CI counter gate B 路径 + D-04 dashboard C 路径 3 子 FULL absorb + R2.4.7 audit hard-fail + Win sentinel = v0.2.0 milestone close 末 phase 完整装配
  - **新 ADR**：0013 doctor-ee4-dashboard-audit-win-sentinel (9 章节 errata 合并 — phase 2.4 ship 时刻 frozen；A7 守恒：ADR 0001-0012 main body 0 diff)
  - **新 baseline tag**：12 → 13 (加 `adr-0013-accepted`；CI A7 step iterate 1-0013 全 13 ADR 守恒)；milestone tag `v0.2.0-alpha.4-doctor` + 🎯 `v0.2.0` 大 tag (Wave 6 T6.4 main agent push)
  - **9 milestone tag 累积**：alpha.1-schema-frozen / alpha.2-installer-runtime / alpha.3-base-profile / alpha.4-routing-engine / alpha.5-routing-l2-engineering / v0.2.0-alpha.1-installers / v0.2.0-alpha.2-execute-task / v0.2.0-alpha.3-extension-mvp / **v0.2.0-alpha.4-doctor** + 🎯 **v0.2.0** (大里程碑 close)
  - **新文件 NEW**：`docs/adr/0013-phase-2.4-doctor-ee4-dashboard-c-path.md` (185L) + `src/cli/lib/origin-check.ts` (~52L sister-share helper) + `src/installers/ccHookAdd.ts` (NEW 7th install method) + `scripts/run-plan-checker.mjs` (~130L walker) + `routing/plan-review-schema.yaml` (NEW 4 维 SSOT) + `manifests/cc-hooks/dashboard-autospawn.yaml` (NEW 第一 cc-hook fixture) + tests `{cli/doctor-fixtures, integration/plan-checker-fixtures, integration/plan-checker-quant, integration/phase-2.4-e2e, routing/ralph-loop-win-sentinel, cli/audit, unit/installers-ccHookAdd}.test.ts` + `.planning/milestones/v0.2.0-{ROADMAP,REQUIREMENTS}.md` + `.planning/v0.2.0-MILESTONE-AUDIT.md` + `.planning/phase-2.4/{2.4-KICKOFF, 2.4-CONTEXT, 2.4-DISCUSSION-LOG, RESEARCH, ASSUMPTIONS, PATTERNS, PLAN, task_plan, PLAN-CHECK, PLAN-CHECK-DELTA, SAMPLES, deferred-items, progress, VERIFICATION}.md`；**修改 MODIFY**：`src/cli/doctor.ts` (152→~215L 5 check) + `src/cli/audit.ts` (扩 origin hard-fail + cmd injection + provenance cross-check) + `src/installers/index.ts` (7-method dispatch table 扩) + `scripts/dashboard.mjs` (~456→610L SSE watcher + 多项目 + localhost-only bind) + `schemas/manifest.v1.schema.json` + `src/manifest/schema/spec.ts` (3 处 enum 加 InstallType/TypeEnum/install.method) + `.github/workflows/ci.yml` (A7 iter 1-0013 + EE-4 step reorder fix + README counter gate + Win sentinel step) + STATE.md + RETROSPECTIVE.md (Phase 2.4 milestone retrospective + v0.2.0 close note 续编) + ROADMAP.md (v0.2.0 4/4 ✅ + v0.3.0 启动 prereq 扩)
  - **关键 finding logged**：T1.1 R2 critical finding "doctor.ts 152L not 38L" (KICKOFF baseline wrong → RESEARCH § 1.2 实测校正) / T2.0 spike outcome 3 阈值 RELAX 1.0 → 0.8 (file_refs 多行 acceptance block 容差 + concrete_acceptance 中英文 quant 关键词 mix) / T3.4 SKIP dashboard.mjs 610L ≤ 650L single-file maintained per B-26 default (proactive split trigger 未到) / T4.1-fix Rule 1 SHELL_EVAL_MARKERS regex refinement (COMMAND_SEPARATORS → real injection vectors only) / deferred-items #3 RESOLVED W6 T6.2 (EE-4 step reorder AFTER pnpm install, 1-line fix)
  - **6 deferred items disposition** (Wave 6 ship-time review cadence): #1 T2.2+T2.4 line-limit ACCEPTED (biome blocking) / #2 phase-2.4 self-check inherent self-referential ACCEPTED (Wave 6 ENFORCE remain false, flip v0.3.0 W0) / #3 EE-4 yaml ERR_MODULE_NOT_FOUND RESOLVED W6 T6.2 (step reorder fix) / T1.1 dual-signal real-API → v0.3.0 (需 ANTHROPIC_API_KEY) / T4.4 Task Session → v0.3.0 (closure infra ready) / EE-4 BLOCKER auto-spawn → v0.3.0 (NOT auto-spawn per D-02 down-scope) / karpathy-skills local-copy install_type → v0.2.4+ (DI-1 Phase 2.3 deferred)
  - **tests baseline**：492+3 → 543+4 (+51 cells across W0-W6)；3-OS CI matrix 全绿
  - 见 `.planning/phase-2.4/{PLAN.md, task_plan.md, progress.md, VERIFICATION.md}` + `docs/adr/0013-phase-2.4-doctor-ee4-dashboard-c-path.md`
- ✅ **Phase 2.3 SHIPPED**（2026-05-16）
  - **31+ atomic 子任务全 ship across 7 Waves W0-W6** (W0 10 task + W1 6 + W2 5 + W3 3 + W4 3 + W5 4 + W6 5 — 含 DI-1 hotfix + STATE/RETRO 续编 + ADR 0012 finalize；详 `.planning/phase-2.3/{progress.md, VERIFICATION.md}`)
  - **Acceptance bar F1-F8 8/8** ✅
    - F1 ADR 0012 errata accepted — `docs/adr/0012-extension-mvp-karpathy-inject.md` Wave 0 draft (T0.2 5 章节 sketch) → Wave 6 fill-out (T6.1 9 章节 expand) → Status `Accepted (phase 2.3 W6 — 2026-05-16)`；9 章节 (5 sketch + 4 W6 fill: § 6 Consequences / § 7 Compliance / § 8 Errata-path note / § 9 Adoption-confirmation)；208L ≤ 250L Karpathy hard limit；39 B-lock cites (B-01~B-39)
    - F2 extension category MVP — 5 NEW manifest 装配 (Wave 1 T1.1-T1.5): `frontend-design.yaml` (design, git-clone-with-setup) + `anthropics-skills-pptx.yaml` + `anthropics-skills-slide-deck.yaml` (content, npx-skill-installer) + `playwright-test.yaml` (testing, npx) + `chrome-devtools-mcp.yaml` (testing, mcp-stdio-add — T1.5 DEVIATION: plan said mcp-http-add, Rule 1 BUG fix no HTTP endpoint upstream); category 3 MVP distribution: design 1 + content 2 + testing 2 = 5 (with existing: 2 + 2 + 3 = 7); Wave 1 T1.6 dry-run e2e integration test 全 PASS
    - F3 karpathy SKILL-ONLY (D-02) — `skills/karpathy-baseline/SKILL.md` NEW (W2 T2.3) + `manifests/skill-packs/karpathy-skills.yaml` REWRITE (W2 T2.4) + `scripts/strip-claude-md-section.mjs` NEW (D-02 migration cleanup cross-platform); **T0.10 always_active spike FAIL** → R2 A1 fallback locked: description-keyword + self-reflexive prompt SKILL.md body 首段 ("ALWAYS apply these 4 心法..."); 0 SDK 改动 (karpathy 4 心法 always-on via SDK skill discovery); CLAUDE.md 不动 (D-02 SKILL-ONLY lock)
    - F4 CD-3 negative-space + arbitrateWithRedirect (W2 T2.1+T2.2+T2.5) — `decision_rules.yaml` 加 9 keywords + 3 negative-space rules (`do_not_use_when` + `if_rejected_use`) + per-manifest hint mirror; `src/routing/lib/arbitrateRedirect.ts` proactive split ≤200L 守住 (B-18); 单一 SSOT (decision_rules.yaml 不开第二来源避免 schema 分裂); arbitrate() legacy 保留避免 byte-stable 回归
    - F5 EE-5 5-question merge gate 双层 (W3 T3.1+T3.2+T3.3) — `src/cli/manifest-add.ts` NEW 78L L1 CLI (commander.js + readline 5Q prompt + `--non-interactive` exit 2 + `--apply` Path A sibling JSON 5-field write); cli.ts register 11 subcommands; `tests/cli/manifest-add-ee5.test.ts` NEW 133L 6 cells; L2 plan-phase 模板 + gsd-plan-checker BLOCKER (KICKOFF § 7 25 answers 5 adapter × 5 题)
    - F6 30-sample routing harness 100% (W4 T4.1+T4.3) — `.planning/phase-2.3/SAMPLES.md` 全新 30 FRESH samples (10 design + 10 content + 10 testing + 5 anchor + 1 false-pos + 4+ CD-3 disqualify edges); `tests/routing/samples-30.test.ts` NEW → **30/30 = 100% hit rate** (≥85% D-05 acceptance bar 远超 15% headroom); D-08 "做出风格" anchor → frontend-design redirect 全命中
    - F7 Wave 0 backlog 6 项一次根治 (W0 T0.3-T0.8) — M1 schema regen drift CI gate (`pnpm build:schema && git diff --exit-code schemas/`) / M2 intel L236 `## 实施进度回填` 节 / **M3 perf gate 根治 D2.3-1 (a)** 移出 CI critical path (`.github/workflows/perf-bench.yml` NEW ≤50L nightly cron advisory + `it.skipIf(IS_GHA)` 守护 — 终止 50→75→100→130→160 累积 nudge anti-pattern) / T1.2 schemaVersion consumer call site gate (B1 honest ≥2 grep `branchOnSchemaVersion`) / T1.3 Win pwsh provenance sentinel (`if: runner.os == 'Windows'` + `shell: pwsh`) / T5 deferred-items review cadence (`scripts/check-deferred-items.mjs` NEW ≤80L warn-only round 1 + ci.yml step + RETROSPECTIVE 模板节)
    - F8 SHIP — `adr-0012-accepted` baseline tag + `v0.2.0-alpha.3-extension-mvp` milestone tag (Wave 6 T6.4 main agent push)；ci.yml A7 step iter `1-11 → 1-0012` (T6.2)；A7 守恒 ADR 0001-0011 main body 0 diff verified (Wave 5 T5.4 mid-wave + Wave 6 T6.4 ship-time 双 verify `git diff adr-0011-accepted..HEAD -- 'docs/adr/000[1-9]-*.md' 'docs/adr/0010-*.md' 'docs/adr/0011-*.md' | wc -l = 0`)；CI 3-OS final verify
  - **7 wave 完整跑完**：Wave 0 (10 task — backlog 6 项根治 + T0.10 spike + STATE/RETRO preheat) / Wave 1 (6 task — 5 NEW manifest + e2e dry-run) / Wave 2 (5 task — decision_rules CD-3 + arbitrateWithRedirect + SKILL.md + karpathy-skills REWRITE + per-manifest hint mirror) / Wave 3 (3 task — manifest-add CLI + register + EE-5 test) / Wave 4 (3 task — SAMPLES.md + T4.2 spike SKIPPED + samples-30 harness) / Wave 5 (4 task — routing-arbitrateRedirect complement + T5.2 SKIPPED + phase-2.3-e2e smoke + A7 mid-wave verify) / Wave 6 (5 task — ADR 0012 9 章节 finalize + ci.yml A7 iter + STATE+RETRO 续编 + baseline tag + DI-1 hotfix + 3 sister review backlog absorb)
  - **delta 1 absorbed**：CD-3 ⭐⭐⭐ negative-space displayed in arbitrateWithRedirect (D-04) — 沿袭 Phase 2.2 delta absorption pattern
  - **新 ADR**：0012 extension-mvp-karpathy-inject (9 章节 errata 合并 — phase 2.3 ship 时刻 frozen；A7 守恒：ADR 0001-0011 main body 0 diff)
  - **新 baseline tag**：11 → 12 (加 `adr-0012-accepted`；CI A7 step iterate 1-0012 全 12 ADR 守恒)；候选 milestone tag `v0.2.0-alpha.3-extension-mvp`
  - **7 milestone tag 累积**：alpha.1-schema-frozen / alpha.2-installer-runtime / alpha.3-base-profile / alpha.4-routing-engine / alpha.5-routing-l2-engineering / v0.2.0-alpha.1-installers / v0.2.0-alpha.2-execute-task / v0.2.0-alpha.3-extension-mvp (Phase 2.3 ship)
  - **新文件 NEW**：`docs/adr/0012-extension-mvp-karpathy-inject.md` (208L) + `src/cli/manifest-add.ts` (78L) + `src/routing/lib/arbitrateRedirect.ts` + `scripts/strip-claude-md-section.mjs` + `scripts/check-deferred-items.mjs` (≤80L) + `skills/karpathy-baseline/SKILL.md` + `manifests/skill-packs/{frontend-design,anthropics-skills-pptx,anthropics-skills-slide-deck,karpathy-skills}.yaml` + `manifests/tools/{playwright-test,chrome-devtools-mcp}.yaml` + `.github/workflows/perf-bench.yml` (≤50L nightly cron) + tests `{routing/samples-30, unit/routing-arbitrateRedirect, cli/manifest-add-ee5, integration/manifest-install-dry-run, integration/phase-2.3-e2e}.test.ts` + `.planning/phase-2.3/{2.3-KICKOFF.md, 2.3-CONTEXT.md, 2.3-DISCUSSION-LOG.md, RESEARCH.md, ASSUMPTIONS.md, PATTERNS.md, PLAN.md, task_plan.md, PLAN-CHECK.md, PLAN-CHECK-DELTA.md, SAMPLES.md, deferred-items.md, progress.md, VERIFICATION.md}`；**修改 MODIFY**：`routing/decision_rules.yaml` (W2 CD-3 fields + 9 keywords + 3 negative-space + per-manifest hint mirror) + `src/cli.ts` (register manifest-add 11 subcommands) + `.github/workflows/ci.yml` (A7 iter 1-0012 + 4 NEW step: schema regen / schemaVersion gate / Win pwsh provenance sentinel / deferred items review cadence) + `tests/integration/manifest-validate.perf.test.ts` (`it.skipIf(IS_GHA)` 守护) + `scripts/check-transparency-verdicts.mjs` (T-W6-1: FRONT_MATTER_DOCS 加 `.planning/STATE.md`) + STATE.md + RETROSPECTIVE.md (Phase 2.3 milestone retrospective 续编)
  - **关键 finding logged**：T0.10 always_active spike FAIL (SDK 0.3.142 无 always_active/auto_load 字段变体 — R2 ASSUMPTION A1 MED-HIGH conf 验证 wrong → fallback description-keyword + self-reflexive prompt) / T1.5 DEVIATION (chrome-devtools-mcp method 选 mcp-stdio-add not mcp-http-add — Rule 1 BUG fix per research § 3.3) / T5.1 DRY consolidation (4-cell spec functionally subsumed by W2 T2.2 d6489bb 6 cells, +3 complementary cells) / T5.2 SKIPPED (Wave 3 T3.3 d6489bb 6 cells already cover spec) / DI-1 hotfix outcome (Wave 6 — install_type:skill + git-clone-with-setup violated ADR 0007 1:N closure → install_type:git + git_ref pin 40-hex all-zeros sentinel; full local-copy install method DEFERRED v0.2.4+)
  - **sister review T-W6-x backlog 全 absorb**：T-W6-1 STATE.md 加入 `FRONT_MATTER_DOCS` (Phase 2.2 T0.4 freshness gate 盲点 closed — STATE.md L4 stale 不再 silent) + T-W6-2 dashboard.mjs (commit `0b4e76d`) RETROSPECTIVE 已完成节回写 (跨 phase reference) + T-W6-3 ADR 0012 5→9 章节 fill (Wave 6 T6.1)
  - **dashboard tooling (T-W6-2 absorb)**：`scripts/dashboard.mjs` NEW (~456L zero-dep ESM read-only `.planning/` + `docs/adr/` + git visualization + tiny inline md→html + mtime polling + hook-ready `--no-open` + port-occupied silent exit) — commit `0b4e76d`，跨 phase tooling reference 跨 phase 2.2/2.3 boundary
  - **tests baseline**：432+3 → 492+3 (+60 cells across W0-W6)；3-OS CI matrix 全绿
  - 见 `.planning/phase-2.3/{PLAN.md, task_plan.md, progress.md, VERIFICATION.md}` + `docs/adr/0012-extension-mvp-karpathy-inject.md`
- ✅ **Phase 2.2 SHIPPED**（2026-05-15）
  - **30 atomic 子任务全 ship across 7 Waves** (W0 7 task + W1 4 + W2 6 + W3 4 + W4 5 + W5 5 + W6 5 = 36 task entries；T4.4 SKIPPED → v0.3.0 / EE-4 deferred → Phase 2.4；详 `.planning/phase-2.2/{progress.md, task_plan.md}`)
  - **Acceptance bar F1-F8 8/8** ✅
    - F1 ADR 0011 errata accepted — `docs/adr/0011-execute-task-sdk-ralph.md` Wave 0 draft (T0.2) → Wave 6 fill-out (T6.1) → Status `Accepted (phase 2.2 W6 — 2026-05-15)`；9 章节 (原 6 + discuss-delta 3：§ 7 SchemaVersion / § 8 Provenance / § 9 Task Session)；380L ≤ 600L；33 B-lock cites ≥ 20；AGENT-DEFINITION-FACTORY-CONTRACT.md main body 0 diff 守恒声明 × 3
    - F2 SDK 引入 + 5/9 字段 contract v1.2 reconcile — `@anthropic-ai/claude-agent-sdk@0.3.142` 入 dependencies + AgentDefinition 5-字段 SDK input (`description / tools? / disallowedTools? / prompt / model?`) + 9-字段 prompt-inject (`skills / mcpServers / memory / maxTurns / background / effort / permissionMode / initialPrompt / criticalSystemReminder_EXPERIMENTAL`)；T1.1 实测 disallowedTools 迁至 SDK input layer 修正 research baseline 4→5 / 10→9；`src/routing/lib/sdkReconcile.ts` NEW 56L；AGENT-DEFINITION-FACTORY-CONTRACT.md main body 0 diff (A7 守恒)
    - F3 dual-signal completion 4-layer — `src/routing/completionSchema.ts` NEW (COMPLETION_SCHEMA + SdkResultEnvelope) + `src/routing/lib/ralphLoop.ts` 42→49L ≤ 50 hard limit (B-26) + isComplete 4-layer (L1 outer PRIMARY structured_output.status='COMPLETE' / L2 outer FALLBACK `<promise>` extract / L3 inner FALLBACK JSON.parse-catch raw / L4 retry)；`systemPrompt.ts` 53→66L COMPLETION_SCHEMA segment + COMPLETE_INSTRUCTION verbatim 不动 (contract § 5.4 1:1 锁)；isComplete 13 cells × 4 layers test cover
    - F4 schemaVersion 7-surface 单一兼容门 (CD-5 ⭐⭐⭐) — `src/types/schemaVersion.ts` NEW 68L (T2.0)；7 surfaces `harnessed.routing-snapshot/handoff-doc/phases-yaml/manifest-state/installer-state/route-decision-log/checkpoint.v1`；3 rules (consumer branch-on-version / unknown enum graceful degrade / 新字段 nested)；`branchOnSchemaVersion<T>()` helper；TypeBox SchemaVersionLiteral；10 unit cells
    - F5 per-phase model tier — `src/workflow/schema/phases.ts` NEW 50L (TypeBox 4-enum ModelTier `haiku/sonnet/opus/inherit` + PhaseEntry + PhasesSchema) + `src/workflow/loadPhases.ts` NEW 30L (YAML parse + Value.Check) + `workflows/execute-task/phases.yaml` NEW 27L (4 phase × intel CD-2 默认表 `[opus, sonnet, sonnet, haiku]`) + 5 cells loadPhases test；`--model-tier inherit` B-10 escape hatch
    - F6 transparency CI gate W3 ENFORCE=true + freshness ext — `scripts/check-transparency-verdicts.mjs` 45→75L (`ENFORCE = true` flip atomic + STATUS_MARKER regex + FRONT_MATTER_DOCS + ROADMAP_LATEST_RE + getLatestShippedToken + checkFreshness)；13 verdict 文档 migration (10 ADD + 3 REPAIR)；Wave 0 ship 后 CI 全绿 verify
    - F7 provenance gate hard fail (CD-6 ⭐⭐) — `provenance.schema.json` NEW 32L (4 字段 source/created_at/confidence/author) + `scripts/check-provenance.mjs` NEW 70L ENFORCE=true walker scope `.harnessed/{sessions,checkpoints,route-logs}/**` (R8 curated paths NOT scanned)；4-step hard-fail synthetic test PASS；CI step 加在 transparency gate 之后 W4 之前
    - F8 SHIP — `adr-0011-accepted` baseline tag + candidate `v0.2.0-alpha.2-execute-task` milestone tag (T6.5 main agent push)；ci.yml A7 step iter `1-10 → 1-0011` (T6.2)；A7 守恒 ADR 0001-0010 main body 0 diff verified (T6.3 `git diff adr-0010-accepted..HEAD -- "docs/adr/000[1-9]-*.md" "docs/adr/0010-*.md" | wc -l = 0`)；CI 3-OS final verify (T6.5 main agent push 触发)
  - **7 wave 完整跑完**：Wave 0 ADR 0011 draft + transparency gate flip + freshness ext + 13 verdict marker migration / Wave 1 SDK 0.3.142 install + smoke import + AgentDefinition 5-字段 verify + SC4 PARTIAL → CD-4 fallback / Wave 2 schemaVersion 7-surface + COMPLETION_SCHEMA + sdkReconcile.ts + systemPrompt segment + ralphLoop.ts 4-layer + dual-signal tests / Wave 3 phases.ts schema + loadPhases.ts + phases.yaml 4 phase 默认表 + load-phases test / Wave 4 provenance gate prep + sdkSpawn.ts NEW + engine.ts real spawn + sdk-spawn.test + routing-engine.test (T4.4 SKIPPED → v0.3.0) / Wave 5 execute-task CLI subcommand + cli.ts register + workflows/execute-task/SKILL.md + SAMPLES.md 30 baseline + execute-task.test + run-samples.mjs harness (30/30 COMPLETE 100%) / Wave 6 ADR 0011 finalize 9 章节 + ci.yml A7 iter 1-0011 + A7 守恒 diff verify + STATE/RETROSPECTIVE 续编 + baseline tag + CI final verify
  - **discuss-phase delta 3 absorbed**：CD-5 FULL schemaVersion (ECC ⭐⭐⭐ → § 7 + T2.0) / CD-6 BEFORE-W4 provenance gate (ECC ⭐⭐ → § 8 + T4.0) / CD-4 PIGGY-W1 Task Session 复用 (omo ⭐⭐ → § 9 + SC4 verify → PARTIAL → fallback DEFERRED v0.3.0)；EE-4 plan 4 维量化阈值 (omo ⭐⭐) → DEFERRED Phase 2.4 doctor 完整版
  - **新 ADR**：0011 execute-task-sdk-ralph (9 章节 errata 合并 — phase 2.2 ship 时刻 frozen；A7 守恒：ADR 0001-0010 main body 0 diff)
  - **新 baseline tag**：10 → 11 (加 `adr-0011-accepted`；CI A7 step iterate 1-0011 全 11 ADR 守恒)；候选 milestone tag `v0.2.0-alpha.2-execute-task`
  - **6 milestone tag 累积**：alpha.1-schema-frozen / alpha.2-installer-runtime / alpha.3-base-profile / alpha.4-routing-engine / alpha.5-routing-l2-engineering / v0.2.0-alpha.1-installers / v0.2.0-alpha.2-execute-task (Phase 2.2 ship)
  - **新文件 NEW**：`docs/adr/0011-execute-task-sdk-ralph.md` (380L) + `src/routing/lib/{sdkSpawn,sdkReconcile}.ts` + `src/routing/completionSchema.ts` + `src/types/schemaVersion.ts` (68L) + `src/workflow/{schema/phases.ts (50L), loadPhases.ts (30L)}` + `workflows/execute-task/{phases.yaml (27L), SKILL.md}` + `src/cli/execute-task.ts` + `scripts/{check-provenance.mjs (70L), run-samples.mjs}` + `provenance.schema.json` (32L) + tests `{routing/sdk-spawn,routing/routing-engine,routing/sdk-reconcile,routing/isComplete,workflow/load-phases,types/schemaVersion,cli/execute-task}.test.ts` + `tests/sdk-import.smoke.test.ts` + `.planning/phase-2.2/{2.2-KICKOFF.md, 2.2-CONTEXT.md, 2.2-DISCUSSION-LOG.md, RESEARCH.md, ASSUMPTIONS.md, PATTERNS.md, PLAN.md, task_plan.md, PLAN-CHECK.md, PLAN-CHECK-DELTA.md, SAMPLES.md, progress.md (待续), VERIFICATION.md (待续)}`；**修改 MODIFY**：`scripts/check-transparency-verdicts.mjs` (45→75L ENFORCE=true + freshness ext) + `src/routing/{engine.ts (199→195L real sdkSpawn integration), agentDefinition.ts (191L unchanged), systemPrompt.ts (53→66L COMPLETION_SCHEMA segment), lib/ralphLoop.ts (42→49L 4-layer + resumeSessionId 闭包)}` + `src/cli.ts` (register execute-task subcommand) + `.github/workflows/ci.yml` (A7 iter 1-0011 + provenance gate step) + `package.json` + `pnpm-lock.yaml` (SDK 0.3.142 + transitive deps) + `schemas/manifest.v1.schema.json` (Phase 2.1 spec.ts regen drift fix) + 13 verdict 文档 (10 ADD + 3 REPAIR marker migration)
  - **关键 finding logged**：T1.1 SDK research baseline correction (4→5/10→9 字段 — disallowedTools 迁移) / T1.2 SC4 PARTIAL outcome (session_id 透传 OK 但 memory+tools state 未完整保留 → B-35 fallback → T4.4 DEFERRED v0.3.0) / T4.2 closure infra 三件套铺设 (sdkSpawn.onSessionId + ralphLoopWrap.resumeSessionId + engine.wrappedSpawn capturedSessionId — v0.3.0 ready 即使 T4.4 consumer DEFERRED) / W2-fix1/2/3 Win+Ubuntu cloud-VM perf gate nudge 75→100→110→130ms (F18b + F38b — Rule 3 auto-fix 3 次累积；同 root cause 与 phase 1.3/1.3.1 一致) / hygiene schemas/manifest.v1.schema.json regen drift fix (Phase 2.1 spec.ts MIT-0/anthropics-official + license_source 未同步到 generated schema.json) / T6.3 A7 守恒 0 diff verify ✅
  - **sister review 反模式根治**：transparency CI gate W3 ENFORCE flip done (Phase 2.1 deferred 兑现) + freshness ext 覆盖 README/PROJECT-SPEC (sister review 洞察兑现) + SSOT 引用纪律 ADR 0011 编号由 plan-phase 实占 (CONTRIBUTING § 项目级规则)
  - **tests baseline**：374+3 → 432+3 (+58 cells across W1-W6)；3-OS CI matrix 全绿
  - 见 `.planning/phase-2.2/{PLAN.md, task_plan.md, progress.md, VERIFICATION.md}`
...
## Phase 2.3 Prereq Notes（Phase 2.2 ship + sister review 后 explicit 启动 prereq — 2026-05-15）

> 来源：Phase 2.2 SHIPPED 后 paranoid sister review (commit 581bc96 已修 H1+H2+H3 + freshness gate root cause)。 以下 M/L tier + T tier 推 Phase 2.3 Wave 0 prereq 处理。

### M tier (engineering 纪律 — Wave 0 prereq 必上)

- **M1 schemas/manifest.v1.schema.json regen drift CI gate 缺失** — Phase 2.2 hygiene commit 18150a5 修 symptom (Phase 2.1 spec.ts MIT-0/anthropics-official/license_source 未同步), 但 RETROSPECTIVE Lesson 6 自我建议 "加 CI step `pnpm build:schema && git diff --exit-code schemas/`" 未实施。**Phase 2.3 Wave 0**: ci.yml 加 step ~10L, 防 spec.ts 改动后 schema.json 漂移复发。
- **M2 intel/omc-comparison.md 实施状态回填** — L236-238 仍写 "Phase 2.2 discuss-phase ⭐⭐⭐ CD-5 / ⭐⭐⭐ CD-2 / ⭐⭐ CD-6" 作 future actionable, Phase 2.2 已 ship 这 3 项。**Phase 2.3 Wave 0**: intel 加 `## 实施进度回填` 节, 每 entry 标 `IMPL: Phase X.Y (commit hash)` 或 `PENDING`。 沿袭 § 0 SSOT 引用纪律。
- **M3 Perf gate 3-tier ADR errata 记录** — F18b/F38b 50→75→100→110→130ms 三次 Rule 3 nudge 累积 across phase 1.3/1.3.1/2.2 已稳定到 3-tier const (CI Win 130 / CI Nix 100 / local 75)。 但无 ADR 记录 cloud-VM-class degrade 根因 + 3-tier 哲学。**Phase 2.3 Wave 0**: ADR 0012 errata (或 0011 errata-of-errata) 记演进哲学, 避免下次 maintainer 误判。
  - **M3 resolved (Phase 2.3 W0 T0.3, commit `<filled>`)** by D2.3-1 (a) 移出 CI critical path — `.github/workflows/perf-bench.yml` NEW nightly cron advisory-only + `tests/integration/manifest-validate.perf.test.ts` 加 `describe.skipIf(IS_GHA)` 守护 (本地 dev 仍跑保 schema-width regression 检测). 累计 nudge 终止 (Win 50→100→130→160 / Nix 50→75→100 final, perf-bench.yml threshold 仍套用但 advisory only). ADR 0012 § 6 + § 9 记 3-tier 哲学.

### T tier (verify deepening + onboarding — Wave 0 prereq 必上)

- **T1.2 schemaVersion consumer call site CI gate** — Wave 2 T2.0 ship `branchOnSchemaVersion<T>()` helper 但未 grep verify 真实 consumer call site。 helper 被 import 不等于被 invoke。**Phase 2.3 Wave 0**: CI gate `grep -r "branchOnSchemaVersion(" src/ | wc -l ≥ 7` (7 surface 各 ≥ 1 consumer)。
- **T1.3 Provenance gate Windows path-sep sentinel** — Wave 4 T4.0 check-provenance.mjs 跑过 4-step synthetic, 但 glob `.harnessed/{sessions,checkpoints,route-logs}/**` 在 Win path-sep 行为未验。 Phase 1.3.1 F37 路径分隔符踩过坑。**Phase 2.3 Wave 0**: ci.yml 加 Windows-specific sentinel `pwsh -c "...check-provenance.mjs..."`。
- **T5 deferred-items.md review cadence 缺失** — D-OOS-1 (`.omc/`) 2026-05-13 → 2026-05-15 三 phase 未 review (H3 已修 .gitignore, 但 process bug 未根治)。**Phase 2.3 Wave 0**: 每次 ship phase 时强制 `cat .planning/phase-*/deferred-items.md` 检查, CI script 或 RETROSPECTIVE 模板加节。

### T tier (Phase 2.3 Wave 6 ship-time backlog from Wave 0+1+2 sister review 2026-05-16)

- **T-W6-1 STATE.md freshness gate root cause** (sister review T5 + H1 同源): Phase 2.2 T0.4 `check-transparency-verdicts.mjs` `FRONT_MATTER_DOCS = ['README.md', 'PROJECT-SPEC.md']` — STATE.md 自身不在 scan 列表, 所以 L4 "最后更新" stale 没被 catch (本次 sister review H1 已 inline fix)。**Phase 2.3 Wave 6 T6.5**: `FRONT_MATTER_DOCS` 加 `'STATE.md'` (1 LOC), 或激进 — 从 STATE.md 删 "最后更新" 行避免与 L25 "状态" 重复成 dual SSOT。
- **T-W6-2 dashboard.mjs STATE 已完成节回写** (M4 sister advisory): commit 0b4e76d dashboard tooling ship 时未在 STATE.md "已完成" 节加 entry。 **Phase 2.3 Wave 6 RETROSPECTIVE 续编时顺便补一行** (dashboard tooling 跨 phase reference)。
- **T-W6-3 ADR 0012 fill 9 章节** (T4 sister advisory + KICKOFF 承诺): 当前 5 章节 (Status / Context / Decisions {1-5+} / A7 / Refs)，沿袭 ADR 0011 fill-out 模板补 4 章节 (## Consequences / ## Compliance / ## Errata-path note / ## Adoption-confirmation)。 **Wave 6 T6.1** 起跑时 reference Phase 2.2 ADR 0011 ship 模板。

### T tier (deferred to later — 非 Wave 0 阻塞)

- **T1.1 dual-signal 4-layer real-SDK error e2e** — 缺 real SDK error 触发 L3 inner FALLBACK 的 e2e cell。 mock JSON.parse 不等于 SDK 输出非 JSON 实际场景。**v0.3.0 prep**: 加 1 real-API integration cell (需 ANTHROPIC_API_KEY)。
- **T1.4 T4.4 DEFERRED onboarding doc** — ✅ DONE (Phase 2.2 sister-review 收尾时 inline) — see `.planning/phase-2.2/T4.4-DEFERRED-onboarding.md`。
- **T2 Phase 2.3 discuss-phase 启动** — 下一步 cadence。 absorb pattern 沿袭 Phase 2.2 f66de16: intel pending CD-3 Do_Not_Use_When + `if_rejected_use:<id>` (L92 "Phase 2.3 extension category MVP 前必上") + EE-5 5-question merge gate 同 discuss 一起拉。
- **T3 v0.3.0 backlog (T4.4 Task Session 复用 完整版)** — closure infra 三件套 ready, ~1 commit (see T4.4-DEFERRED-onboarding.md)。 Phase 2.3 ship 后 v0.3.0 启动时即可 land。
- **T4 Phase 2.4 backlog (EE-4 plan 4 维量化阈值 schema)** — Phase 2.4 doctor 完整版 absorb OR phase 2.5 独立 — Phase 2.3 plan-phase 时决断。

---

## Phase 2.4 Prereq Notes（Phase 2.3 ship + sister review post-mortem 后 explicit 启动 prereq — 2026-05-16）

> 来源: Phase 2.3 SHIPPED post-mortem sister review (commit 161621c 之后)。 H1+H2 inline fix README counter+entries 即时 ship; 其余 H3/M1/M2/T2/T3 推 Phase 2.4 Wave 0 prereq backlog 一次根治。

### H tier (engineering 纪律 — Wave 0 prereq 必上)

- **H3 + T4 README counter integrity CI gate** (sister review root-cause-class 第 3 次复发 — Phase 2.2 / Phase 2.3 W3-prereq / Phase 2.3 ship 后)。 FRONT_MATTER_DOCS 只检测 Status: marker, 不验 README enumeration (Phase X shipped 行数 vs Acceptance bar X1-X8 行数 vs L44 counter)。 **Phase 2.4 Wave 0** 两候选择 (a) ADR errata 收敛 README enumeration 到 STATE.md 单 SSOT (删 README 冗余) OR (b) CI gate `grep "Phase 2.X shipped" wc -l` vs counter vs bar 三者一致才 PASS。 (a) 更激进 + Karpathy 简单 + SSOT 单源; (b) 更保守 + README 用户面信息保留。 plan-phase 决断。

### M tier (advisory / 长期一致性)

- **M1 RETROSPECTIVE dashboard polish round 1 entry** — RETRO 提了 0b4e76d NEW 跨 phase tooling reference, 但没单提 161621c polish round 1 (7 项 UX + 1 silent bug fix + rename Dashboard→STATE)。 advisory only — polish 是 NEW 延伸, 不另立功能 entry 合理。 **Phase 2.4 RETRO** 补一句历史 cluster: "dashboard tool: 0b4e76d NEW → 161621c polish round 1 → ... (Phase 2.4 doctor 完整版 C 路径 absorb 候选)"。
- **M2 CD-5 schemaVersion 7 surface long-tail** — intel 写"7 surface, current consumer = 2"。 还有 5 surface 待 land (handoff-doc / phases-yaml / manifest-state / installer-state / route-decision-log / checkpoint / agent-definition-factory)。 **Phase 2.4+** 每 phase 至少 land 1 consumer 避免 "声明 7 但实际 2" 长期挂账。

### T tier (Phase 2.4 discuss-phase 决策点)

- **T2 dashboard tool 进 Phase 2.4 doctor 完整版 absorb 评估** — `.planning/intel/dashboard-handoff-2026-05-16.md` § 3 C 路径已 advisory (SessionStart hook auto-install + STATE.md watcher + 多项目支持, 共 ~160L 增量)。 Phase 2.4 discuss-phase 决断: absorb 进 doctor 完整版 sister cluster OR defer 到 v0.3.0。
- **T3 v0.3.0 backlog T4.4 Task Session 启动准备** — closure infra 三件套 ready (sdkSpawn.onSessionId / ralphLoopWrap.resumeSessionId / engine.wrappedSpawn capturedSessionId), consumer 接入 + `harnessed.phases.v1` schema bump 加 `task_session_id?` field 即可。 **Phase 2.4 ship 之后** v0.3.0 立刻启动候选。

### Already absorbed / no action

- **M3 task_plan.md 1306L 巨大** — inherent pattern, Phase 2.3 ship 后 archive (已自然完成 via Wave 6 ship)。 advisory only, 无需 action。
- **T1 RETROSPECTIVE Next Phase Prep Notes** — Phase 2.3 RETRO L72-L133 已包含 Next Phase Prep Notes 节 (sister precedent Phase 2.2 RETRO 格式)。 verify pass。
- **T5 .planning/phase-2.4/ 待创建** — 正常路径 (由 /gsd-discuss-phase 2.4 触发); dashboard Current Phase view 自动识别最新 phase-X.Y。 无需预动。

---

## v0.3.0 启动 prereq（Phase 2.4 Wave 0 T0.5 预启动注记 — 2026-05-16）

> Phase 2.4 ship 后 (= v0.2.0 milestone 4/4 close) v0.3.0 立刻启动候选 backlog。 advisory only — 不预 land；Phase 2.4 ship 后 v0.3.0 discuss-phase 时正式 absorb。

### closure infra 三件套 ready (Phase 2.2 T4.1/T4.2/T4.3 ship)

- **`sdkSpawn.onSessionId`** — SDK first message session ID capture closure
- **`ralphLoopWrap.resumeSessionId`** — resume from captured session ID closure (sister T4.4 DEFERRED)
- **`engine.wrappedSpawn capturedSessionId`** — engine-level wrap that propagates session ID to handoff doc

v0.3.0 consumer 接入 = 给 `harnessed.phases-yaml.v1` schema bump 加 `task_session_id?` field + 7 surface long-tail land (current consumer = 3 after Phase 2.4 W0 T0.4 + 7 - 3 = 4 surface 仍 long-tail)。

### v0.3.0 候选 features

- **T3 plan-feature workflow + checkpoint cadence** — gstack-mediated cross-phase gate enforcement; checkpoint 写入 `.harnessed/checkpoints/` per ADR 0011 § 8
- **EE-4 BLOCKER auto-spawn rerun** — Phase 2.4 down-scope manual rerun → v0.3.0 自动 subagent loop (依赖 ralph-loop full integration 稳态 + subagent budget gating)
- **路由命中率验收** — 30/30 SAMPLES.md FRESH-N reset 模式扩到 cross-version routing accuracy gate (sister Phase 2.3 SAMPLES.md 100% routing 模板)
- **gstack 前缀探测** — `/gstack:office-hours` / `/review` 等 slash command 自动 detect + propagate to harnessed routing
- **Task Session 复用 完整版** — T4.4 DEFERRED → consumer 接入 + schema bump + 完整 lifecycle (resume-restart-cleanup)

### Trigger

Phase 2.4 ship + `v0.2.0` milestone tag pushed → v0.3.0 discuss-phase 启动 (`/gsd-discuss-phase 3.0` 或 sister 命名)。 sister Phase 2.2 → Phase 2.3 → Phase 2.4 cadence 模式。
