...
> **execute-phase 工作量**: 5-7 工作日（v3 ROADMAP § Phase 1.3 行 — H3 sister patch transparency 修订；历史 phase 1.1/1.2 实际比初估时长，phase 1.3 大概率也会）
...
Wave 2 (decision_rules.yaml v1 + arbitrate logic, 90 min)
  T3.1 routing/decision_rules.yaml v1 起草 (6 category × ≥ 12 rules)
...
Wave 6 (Cross-OS CI verify, push verify)
  ...
  T7.3 (H1b sister patch) Perf attribution — profile manifest validate 路径，
       对比 v0.1.0-alpha.1 (21.7ms) vs alpha.2 vs phase 1.3 当前 (~28ms)，识别
       schema 加 3 字段贡献的 perf cost 量化；输出 .planning/phase-1.3/PERF-ATTRIBUTION.md
       (≥ 50 行) — Wave 6 push verify 时执行
...
| 6 | T7.1-T7.3 | 3 | CI verify + 可能 hotfix + perf attribution (H1b sister patch) |
...
| `routing/decision_rules.yaml` v1 schema | T3.1 + T3.2 | routing engine load + parse |
...
4. **B4** `routing/decision_rules.yaml` 通过 Ajv strict validate + arbitrate function 单测覆盖（≥ 8 cell）
