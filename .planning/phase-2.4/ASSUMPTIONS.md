...
| **B-36** | **ADR 编号 = plan-phase 实占,绝不预占 `0013`** (intel § 0 + CONTRIBUTING.md 项目级 SSOT 引用纪律 + D2.4-20) — 本 phase 所有文档 (KICKOFF/PATTERNS/RESEARCH/ASSUMPTIONS/PLAN/task_plan/NEW ADR/PLAN-CHECK) 写 `<实占N>` placeholder;**禁写 `ADR 0013`**;T0.1 sed-replace discipline 批量 resolve (Phase 2.1 W1 + Phase 2.3 T0.1 plan-check fix 沿袭) | PATTERNS § 5 + RESEARCH D2.4-20 + KICKOFF § 3 Hard Constraint #2 |
| **B-37** | **A7 守恒持续**:ADR 0001-0012 main body **0 diff** (Phase 2.4 新 ADR 走 errata 路径);ship 时 baseline tag `1-12 → 1-<实占N>`;CI `ci.yml` A7 step iter 同步 (iter 1-12 → 1-N);`docs/AGENT-DEFINITION-FACTORY-CONTRACT.md` + `docs/INSTALLER-CONTRACT.md` main body 不动 | KICKOFF § 3 Hard Constraint #1 + #10 |
...
| **B-40** | TypeBox not zod — schema 改动用 `@sinclair/typebox` `Type.*`;沿袭 ADR 0010/0011/0012 errata 注释块 fence 模式;新 ADR errata 注释行 `// ADR <实占N> errata — <topic> (phase 2.4 W<N> — F<N>)`;3 处 enum 加项 (InstallType + TypeEnum + install.method) 全 TypeBox additive | KICKOFF § 3 Hard Constraint #3 |
...
| Lock | Decision | Source chain |
|------|----------|--------------|
| **B-42** | Wave 6 ship 三 tag (沿袭 Phase 1.5 v0.1.0 close 模式):(a) `adr-<实占N>-accepted` baseline tag;(b) `v0.2.0-alpha.4-doctor` candidate tag (v0.2.0 alpha cycle 末 alpha);(c) **`v0.2.0` milestone tag** (大里程碑 — v0.2.0 alpha cycle close,4/4 phase ship 完结) | KICKOFF § 1.2 F8 + Phase 1.5 v0.1.0 close sister precedent |
