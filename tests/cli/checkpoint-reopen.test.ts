...
// One stateful record shared by every mocked state primitive, and
// `writeCurrentWorkflow` modelled as what it really is: a WHOLE-RECORD REPLACE.
//
// The previous mock made writeCurrentWorkflow a no-op, which is exactly why a
// self-inflicted rollback survived here: reopen wrote the pre-reopen snapshot
// back through it, and a no-op write could never overwrite the ledger the other
// mock had just changed. A mock that is kinder than the real thing hides the
...
const seed = (entries: SubProgressEntryType[], status: 'active' | 'complete' = 'complete') => {
  ...
  const sync = () => {
    ledger = record?.sub_progress ?? []
  }
  sync()
  vi.mocked(readCurrentWorkflow).mockImplementation(async () =>
    record ? structuredClone(record) : null,
  )
  vi.mocked(mutateSubProgress).mockImplementation(async (fn) => {
    if (!record) return
    record = { ...record, sub_progress: fn(record.sub_progress ?? []) }
    sync()
  })
  vi.mocked(writeCurrentWorkflow).mockImplementation(async (next) => {
    record = structuredClone(next) // whole-record replace, like the real one
    sync()
  })
  vi.mocked(mutateWorkflow).mockImplementation(async (fn) => {
    if (!record) return
    record = fn(structuredClone(record))
    sync()
  })
}
...
describe('checkpoint reopen', () => {
  ...
  it('cell 1 — reopens a done sub, exit 0, ledger mutated', async () => {
    ...
    expect(mutateWorkflow).toHaveBeenCalledTimes(1)
  ...
  it('cell 3 — a COMPLETE workflow is flipped back to active (verify runs after the chain closed)', async () => {
    ...
    // The regression: with the flip done as a second, whole-record write of the
    // pre-reopen snapshot, status came out active while the sub came out `done`
    // again — reopen undone by its own follow-up write.
    expect(ledger.find((x) => x.sub === 'impl')?.status).toBe('pending')
  })
