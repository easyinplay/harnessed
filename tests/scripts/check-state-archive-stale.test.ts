...
describe('check-state-archive-stale.mjs — D3 3-rule ENFORCE round 2 + SIZE_LIMIT=150 round 3 FLIP (Phase 5.2 W0 T0.2 #BA resolve)', () => {
  it('1. compliant STATE.md ≤150L + no violations → exit 0 + stdout "compliant"', () => {
    // 149L compliant fixture, no key-decisions sections, no W-N errata literals
    const lines = ['# STATE.md', '## Current Position', 'Phase 5.2 in progress']
    for (let i = 0; i < 146; i++) lines.push(`- item ${i}`)
  ...
  it('2. Rule 1 violation — STATE.md 250L > 150L → exit 1 (ENFORCE round 2 + SIZE_LIMIT=150) + stderr "Rule 1 (size)"', () => {
    // 250L STATE.md to trigger Rule 1 (size) violation
    const lines = ['# STATE.md', '## Current Position']
    for (let i = 0; i < 248; i++) lines.push(`- line ${i}`)
    writeStateMd(lines.join('\n'))
    const r = runScript()
    expect(r.code).toBe(1) // ENFORCE round 2 + SIZE_LIMIT=150 round 3 FLIP (Phase 5.2 W0 T0.2 #BA resolve)
    expect(r.stderr).toMatch(/Rule 1 \(size\)/)
    expect(r.stderr).toMatch(/250L > 150L/)
  })
