...
> AI coding harness 套件管理器 + Composition Orchestrator
> 將三層架構協作方法論（gstack 治理 + GSD 專案管理 + superpowers 資深工程師 + karpathy 原則 + mattpocock 招式）機器化執行為可運行的引擎
...
**針對 Claude Code 工程實踐的最佳作法 Orchestration** — 整合最優秀的開源 Claude Code 生態元件，透過帶有主見的 Composition Skills 將其編織成統一的 Workflow；不 Vendor 上游程式碼 — Manifest 負責描述安裝與檢查，Composition Skills 則負責協調多個上游元件的協作。
...
- **三層架構機器化執行** — `gstack 治理` + `GSD 專案管理` + `superpowers 資深工程師` + `karpathy 4 原則` + `mattpocock 23 招式`，五大支柱 100% 完整覆蓋
- **不 Vendor 上游** — Manifest 描述安裝與檢查；上游升級時，使用者只需重新安裝即可取得最新版本
- **Composition Skill** — 自製 Workflow Skills 作為指揮棒，協調多個上游元件協同演奏。**1 個超級主控 `/auto` + 4 個階段主控 + 18 個子 Workflow + 2 個獨立工具 = 25 個命名空間分層 Workflow**，完整四階段機器化執行（`/auto` 跨階段一鍵完成 / `/discuss /plan /task /verify` 單一階段 / 18 個三層架構子項 / `/research /retro` 2 個獨立工具）
- **L0 行為準則基底** — 全域跨階段行為基準（karpathy 原則 + 輸出風格 + 語言 + 操作規範 + 優先級 + 協議），普遍套用
- **套件管理思維** — 安裝相依圖自動解析、doctor 健康檢查、install-base 一鍵完整安裝
- **統一進入點** — 使用者只需面對 `/discuss /plan /task /verify` 主指令，無需學習各上游的術語；子指令可明確呼叫單一階段（例如 `/discuss-strategic` 只執行策略層澄清）
...
**解除安裝 harnessed：**
```bash
harnessed uninstall    # 移除 harnessed 自身檔案（上游元件不受影響）
```

> `harnessed uninstall` 會清理 commands、workflow skills、settings 環境變數和狀態目錄。上游元件（npm 套件、MCP 伺服器、CC 插件、git clone 倉庫、npx skills）保持完整。執行 `harnessed uninstall <name>` 可單獨移除某個上游。加 `--dry-run` 可預覽。
...
### 🎯 Auto 模式（推薦給新手 / 不想費腦筋的人）

```
/auto "需求 X"

# 對於大型需求可明確分階段執行（通常不需要 — AI 會自動判斷並路由；
# 若你認為需求較大，可強制使用）：
/auto "需求 X" --staged
```

> 不想費腦筋，或剛開始上手 — 讓 harnessed 全權處理。執行完整 6 個階段（research 條件觸發 → discuss → plan → task → verify → retro 強制執行），中途不停頓。AI 一鍵自動判斷需求複雜度，針對大型需求建議切換至 `--staged` 模式（每個階段後暫停供審查）；開始前提示「你對需求是否有清楚的理解？」— 若否 → 自動執行 `/research` 多來源調查；以強制執行的 `/retro` 摘要作結。遇到失敗立即中止，可透過 `harnessed resume` 繼續。

### 📂 分階段模式（推薦給進階使用者 / 想審查中間結果的人）

```
/discuss "需求 X"          # 策略 + Phase + 子任務三層澄清
/plan "需求 X"             # 架構（條件觸發）+ 計畫持久化
/task "子任務-1"           # 4 個子項串行執行（clarify → code → test → deliver）
/verify "phase-1"          # 7 個子項條件驗證
```

> 想自行決定從哪個階段開始 / 審查中間輸出 — 4 個主控可獨立呼叫，每個主控內部仍會自動展開該階段的所有子項。
...
# ... 從其他 18 個子 Workflow 中任選
```

> 「我是專家，我自己決定」— 跳過主控，直接呼叫子 Workflow。適合知道自己需要哪個子項的進階使用者，或單步驟的複用場景。
...
> 虛線框 = 選用獨立工具（`/research` 前置策略調查 / `/retro` 里程碑後摘要）；實線框 = 主要四階段流程。

### 25 個 Workflow 總覽表

| 指令 | 階段 | 類型 | 能力 / 上游 | 簡介 |
|-----------|-------|------|----------------------|-------|
| `/auto` | 全部 | **超級主控** | masterOrchestrator（跨 6 個階段） | 一鍵完整執行 6 個階段（research 條件觸發 → discuss → plan → task → verify → retro 強制執行）；AI 一鍵複雜度判斷 + 理解確認 + 強制 retro；`--staged` 選擇性啟用階段關卡 |
...
| `/plan` | ② Plan | 主控 | masterOrchestrator | 串行呼叫 2 個子項（architecture 條件觸發 → phase 永遠執行） |
...
| `/task-clarify` | ③ Task | 子項 | superpowers Brainstorming + `/grill-with-docs` 條件觸發 | 子任務啟動澄清關卡 |
| `/task-code` | ③ Task | 子項 | karpathy 4 原則 + `/zoom-out` / `/improve-codebase-architecture` / `/diagnose` 條件觸發 | 子任務編碼 + 跨 session 的 progress.md 同步 |
| `/task-test` | ③ Task | 子項 | superpowers TDD red-green-refactor + `/diagnose` 條件觸發 | 核心邏輯 TDD 強制執行（別名 mattpocock `/tdd`） |
| `/task-deliver` | ③ Task | 子項 | `ralph-loop` SDK 包裝器 + Agent Teams 條件觸發 | 直到逐字輸出 `COMPLETE` + R20.10 max_iter 退路 |
...
| `/verify-qa` | ④ Verify | 子項 | gstack `/qa` + playwright-cli / `@playwright/test` / webapp-testing | 端對端 QA（has_ui_changes 條件觸發） |
| `/verify-security` | ④ Verify | 子項 | gstack `/cso` | OWASP / 身份驗證 / 機密（has_auth_or_secrets 條件觸發） |
| `/verify-design` | ④ Verify | 子項 | gstack `/design-review` + ui-ux-pro-max + frontend-design | 設計系統一致性（has_design_changes 條件觸發） |
...
四階段三層架構方法論 — 建議依序使用 4 個主控 Orchestrator 驅動：
...
| 階段 | 主控 | 主要子 Workflow | 上游協作 |
| ---- | ---- | ---- | ---- |
| ① **Discuss** | `/discuss` | strategic / phase / subtask（3 個平行） | gstack `/office-hours` + GSD `/gsd-discuss-phase` + superpowers Brainstorming |
| ② **Plan** | `/plan` | architecture（條件觸發）→ phase | gstack `/plan-eng-review` + GSD `/gsd-plan-phase` + planning-with-files |
| ③ **Task** | `/task` | clarify → code → test → deliver（每個子任務 4 個串行） | karpathy 原則 + mattpocock 招式 + superpowers TDD + `ralph-loop` |
| ④ **Verify** | `/verify` | progress → 5 個平行條件觸發 → simplify（+ multispec 關鍵時） | GSD `/gsd-verify-work` + code-review + gstack `/review` / `/qa` / `/cso` / `/design-review` + code-simplifier |
...
# 2. 在 Claude Code 內執行四階段流程
/discuss "新功能 X"          # 策略 + Phase + 子任務三層澄清
/plan "新功能 X"             # 架構（條件觸發）+ 計畫（任務圖持久化）
...
├── workflows/                  # L6：Composition Skills（四階段指揮棒）
...
│   /discuss /plan /task /verify（主控）+ 18 個子項 + /research /retro + /auto 超級主控
...
tool-slash-cmd (~60): gstack 30+ 選用 + gsd 10+ + mattpocock 12 高頻 + 等
...
[L0] discipline.priority-hierarchy 仲裁工具衝突 / output-style 格式化輸出
  ↓
[fires=true 子項] → 呼叫子 Workflow（/discuss-strategic / /discuss-phase / /discuss-subtask）
  ↓ 對每個子項：
...
| `harnessed uninstall [name]` | 統一解除安裝 — 不加名稱：移除 harnessed 自身檔案（上游完整保留）；加名稱：移除單一上游元件 |
...
```bash
harnessed setup  # 自動安裝 gstack + GSD + superpowers + planning-with-files；25 個 Workflow Skills 部署至 ~/.claude/skills/ + Agent Teams 環境變數自動寫入 ~/.claude.json
```
...
4. **單點故障** — 一個維護者同步維護 4-5 個 Vendor 上游 = 加速 burnout
...
`/discuss /plan /task /verify` — 4 個主控將 4 個階段串接起來；每個主控在內部委派給對應的子項。每個階段做不同的事，並為下一個階段提供輸入。**不合併**。
