...
| `upstream_health.last_known_good_version` | ✅ | string | 上次核实时观察到的上游版本。**取值来源按 `install.method` 分**：`cc-plugin-marketplace` 取 marketplace 清单（`.claude-plugin/marketplace.json`）声明的 plugin 版本，**不是** repo 的 git tag（tag 可能跑在已发布 marketplace 条目前面）；npm 类取 registry 的 `latest`；git 类取 tag 或 HEAD sha。写错来源会让 doctor 的 `plugin install freshness` 对一个渠道根本不供的版本永久报「落后」 |
