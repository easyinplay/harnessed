...
- Vulnerabilities in **upstream packages** themselves (gstack / GSD / Tavily MCP / Exa MCP / ctx7 / planning-with-files / superpowers / mattpocock-skills) — report to those projects directly. harnessed only describes them via manifests; trust delegation is by design (see `THREAT-MODEL.md` when v0.4 ships).
