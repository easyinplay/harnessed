...
function detectLoop(ledger) {
  return ledger.filter((e) => (e.fail_count ?? 0) >= LOOP_THRESHOLD).map((e) => ({ sub: e.sub, count: e.fail_count }));
}
...
function buildInjection(repoRoot, wf, learningsMd, budget = DEFAULT_INJECT_BUDGET, intent = null, nowMs = Date.now(), opts = {}) {
...
function harnessedRoot() {
  ...
  return override !== void 0 && override !== "" ? override : join4(homedir(), ".claude", "harnessed");
}
