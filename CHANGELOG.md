...
- **capabilities 的 `requires.settings_env_var` / `requires.cc_version` / `sdk_ref`。** 三者从未被读取。`cc_version` 已与代码漂移(见上方 Fixed),Agent Teams 前置条件现在只有一个家:`checkAgentTeams.ts` 的探测与提示。`sdk_ref` 是存成数据的代码导航注释,且已腐烂 —— `completion-gate` 的 `sdk_ref` 在 ADR 0039 之后仍指向 `ralphLoop.ts`,实际实现是 `harnessed checkpoint complete`。

闸门豁免表至此从 25 条回到 11 条(Phase 61 的 10 条 + 刻意保留为文档的 `routing_note`)。
...
- **删 `manifests/tools/exa-mcp.yaml`,exa 由 ECC MCP catalog opt-in 承接。** ECC(`manifests/optional/ecc.yaml`,optional 档)的 MCP catalog(`mcp-configs/mcp-servers.json`,手动拷入 `.mcp.json`)提供 hosted exa MCP(`https://mcp.exa.ai/mcp`,同名工具 `web_search_exa` / `web_fetch_exa`;≤2.0.0-rc.1 旧版曾默认 bundle,上游 2026-06 MCP connector audit 后退役为 opt-in),setup Step B 自动 glob 安装 exa-mcp manifest 与之重复且钉旧。`exa-mcp` capability **保留**(3 处 yaml 引用 + check-workflow-schema cross-validate),description 注明 provider = ECC catalog opt-in;**fallback 语义**:未 opt-in 时 exa capability 不可用,web-search-routing 的 exa 分支不 fire,落 tavily-mcp 默认分支 / 内置 WebSearch(既有 fallback 分支,行为零回退)。连带:`tavily-mcp.yaml` `upstream_health.alternative: exa-mcp → ecc`;doctor 12th check `checkMcpAvailability` TARGET_SERVERS 收窄为 `tavily-mcp`(check 名 `MCP servers (tavily)`);CI 冒烟 exa-mcp dry-run cell 与 mock-claude `mcp list` 的 exa 行移除。
- **`manifests/tools/chrome-devtools-mcp.yaml` 改装官方 plugin(`cc-plugin-marketplace`)。** 原 mcp-stdio-add 钉死 npm `chrome-devtools-mcp@^0.1.0`(旧);官方内置 marketplace `claude-plugins-official` 已有 chrome-devtools-mcp plugin(自动更新、工具更全)。改:`type: cc-plugin` / `install_type: skill`(validate 层 install_type↔method 1:N 闭合中唯一允许 cc-plugin-marketplace 的值)/ `component_type: mcp-tool`(plugin 交付物仍是 MCP 工具);install cmd 单条 `claude plugin install chrome-devtools-mcp@claude-plugins-official`(内置 marketplace 无需 `marketplace add`,`marketplace_source` 省略);`git_ref: 1.6.0`(本机 marketplace cache 实测最新);verify/uninstall 走 `claude plugin` 命令。capability 名/触发不变(web-testing-routing `perf-a11y-memory` 路由零变化)。codex 语义:codex 无 claude plugin 生态,经 `harness_overrides.codex` 保留 mcp-stdio 路径(`npx chrome-devtools-mcp@^1.6.0`,顺带解旧 manifest 钉死 ^0.1.0)+ codex 分发测试 cell,消 plugin 化引入的 harness-mismatch 回归。另注:ECC 默认 bundle 同名 chrome-devtools connector,装 ecc 的用户以 `ECC_DISABLED_MCPS=chrome-devtools`(ECC install/sync 期过滤)避免双份(manifest notice 已注)。
- **`manifests/optional/ecc.yaml` 补 codex sync 通道(`harness_overrides.codex`)。** ECC 多 harness:CC 走 plugin marketplace(既有 cmd 不变);codex 走上游 README 推荐的 sync flow(clone + `bash scripts/sync-ecc-to-codex.sh`,merge AGENTS.md/skills/prompts/agents 进 `~/.codex`,timestamped backup;codex marketplace 路线 experimental 不推荐)。override 用 `git-clone-with-setup`(gstack 式 pre-clean 保留 clone cache 供 D-15 SHA-verify);idempotent/verify 为弱校验(探 clone cache 存在性,~/.codex merge 态无稳定 artifact 契约,manifest 注释已注明)+ codex 分发测试 cell。
- **`manifests/skill-packs/ui-ux-pro-max.yaml` 改装官方 plugin(`cc-plugin-marketplace`)。** 原 git-clone-with-setup 自打包(clone midwayjs/midway v4-next 拼 `.codex/skills/ui-ux-pro-max` 进 `~/.claude/skills/`);官方 plugin 渠道已存在(nextlevelbuilder/ui-ux-pro-max-skill marketplace),自打包与之双载。改:`type: cc-plugin` / `install_type: skill` / `component_type: command` 不变;第三方 marketplace ⇒ 两段式 cmd(`claude plugin marketplace add nextlevelbuilder/ui-ux-pro-max-skill && claude plugin install ui-ux-pro-max@ui-ux-pro-max-skill`,sister karpathy-skills.yaml)+ `marketplace_source`;`git_ref: 2.5.0`(本机 marketplace cache 实测);verify/idempotent/uninstall 走 `claude plugin` 命令;codex 经 `harness_overrides` 保留 midway git-clone 路径(`claude plugin` 为 Claude Code-only)。decision_rules 路由零变化。测试:git-clone-idempotent 的 ui-ux-pro-max cell 移除,dry-run 断言迁入 manifest-install-dry-run(expectedMethod `cc-plugin-marketplace`)+ codex 分发测试 cell。
- **mattpocock 招式名对齐上游 2026-07 重构(engine/doctor/README ×10)。** 上游改名 `/diagnose` → `/diagnosing-bugs`、`/to-prd` → `/to-spec`、`/to-issues` → `/to-tickets`,删除 `/zoom-out`。engine(capabilities.yaml alias 键 `diagnose` → cmd `/diagnosing-bugs`)与 doctor(check-mattpocock-skills 探测新名 + legacy fallback)已在 4.16.x/v15.0 落地;本次收尾传播:README.md 及 9 个语言 sibling(cn/tw/ja/ko/pt-BR/tr/ru/vi/th)招式列举改新名、`/zoom-out` 列举项移除(叙述句示例以 `/grill-with-docs` 顶替);workflows/task/test SKILL.md + SKILL.zh-Hans.md + workflow.yaml 注释中的 `/diagnose` 改 `/diagnosing-bugs`。.planning/ 与 docs/adr 历史文档不动。
