...
- **新项目引导**: `.planning/ROADMAP.md` 缺失时 stage ① 先跑 `/gsd-new-project`(可用时)或先创建 ROADMAP/STATE/REQUIREMENTS 最小骨架
- **Context 自动传递**: planning-with-files `.planning/phases/<NN>-<slug>/` 喂给下游 stage
...
**不要** pipe 到 `harnessed run auto` —— 那是 CI/headless 路径({{ host.harnessed_run_warning_note.orchestrator_tail }}
