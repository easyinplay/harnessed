...
保 completion-promise (bundled subagent vs {{ host.team.attributive }} routing — orthogonal wrapper rule).
...
Do NOT pipe to `harnessed run task` — that is the CI/headless path ({{ host.harnessed_run_warning_note.orchestrator_tail }}
